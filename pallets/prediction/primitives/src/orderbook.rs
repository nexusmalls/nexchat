...
/// Stable identifier for an orderbook order.
/// 订单簿订单的稳定标识符。
pub type OrderId = u128;

/// A maker order exchanging a market base asset and one outcome asset.
/// maker 在市场基础资产与一个结果资产之间交换的订单。
#[derive(
    Clone,
    Encode,
    Eq,
...
{
    /// Returns the fixed-point price of the order in the supplied base asset.
    /// 返回以指定基础资产计价的订单定点价格。
    ...
    /// 返回按 `sub_maker_amount` 成交时 taker 与 maker 各自需要提供的数量。
    ///
    ...
    /// 返回按 `sub_taker_amount` 成交时 taker 与 maker 各自需要提供的数量。
    ///
...
mod tests {
    ...
    fn taker_and_maker_fill_from_maker_amount_works(
        ...
        let (taker_fill, maker_fill) = order
            .taker_and_maker_fill_from_maker_amount(sub_maker_amount)
            .unwrap();
    ...
    fn taker_and_maker_fill_from_taker_amount_works(
        ...
        let (taker_fill, maker_fill) = order
            .taker_and_maker_fill_from_taker_amount(sub_taker_amount)
            .unwrap();
        assert_eq!(taker_fill, expected_taker_fill);
        assert_eq!(maker_fill, expected_maker_fill);
    }
