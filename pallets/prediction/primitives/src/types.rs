...
/// An index to a block. Aligned with Nexus runtime `BlockNumber`.
/// 区块索引，与 Nexus runtime 的 `BlockNumber` 对齐。
...
#[derive(TypeInfo, Clone, Debug, Decode, DecodeWithMemTracking, Encode, Eq, PartialEq)]
pub enum MultiHash {
    Sha3_384([u8; 50]),
}
