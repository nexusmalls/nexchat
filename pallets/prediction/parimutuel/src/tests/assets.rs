...
//
// This tracked upstream test module is intentionally not registered by
// `tests/mod.rs`, matching the selected upstream snapshot.
...
fn destroyed_losing_after_resolution_with_winner() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        <Runtime as Config>::AssetDestroyer::on_idle(System::block_number(), Weight::MAX);
        assert!(<Runtime as Config>::AssetCreator::asset_exists(
            winner_asset.into()
        ));
        ...
        for asset in market
            .outcome_assets()
            .iter()
            .filter(|asset| Asset::from(**asset) != Asset::from(winner_asset))
        {
            assert!(!<Runtime as Config>::AssetCreator::asset_exists(
                (*asset).into()
            ));
...
fn destroyed_after_resolution_without_winner() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        for asset in market
            .outcome_assets()
            .iter()
            .filter(|asset| Asset::from(**asset) != Asset::from(losing_asset))
        {
            assert!(!<Runtime as Config>::AssetCreator::asset_exists(
                (*asset).into()
            ));
