...
fn build_market_interacts_correct_with_latest_market_id_and_returns_correct_values() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            market,
            builder.market_id(market_id).clone().build().unwrap()
        );
