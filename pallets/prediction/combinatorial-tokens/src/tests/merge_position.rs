...
#[test_case(
    Asset::Ztg,
    Some(CombinatorialToken([207, 168, 160, 93, 238, 221, 197, 1, 171, 102, 28, 24, 18, 107, 205, 231, 227, 98, 220, 105, 211, 29, 181, 30, 53, 7, 200, 154, 134, 246, 38, 139])),
    Some(CombinatorialToken([101, 210, 61, 196, 5, 247, 150, 41, 186, 49, 11, 63, 139, 53, 25, 65, 161, 83, 24, 142, 225, 102, 57, 241, 199, 18, 226, 137, 68, 3, 219, 131]))
)]
#[test_case(Asset::ForeignAsset(USDX_ASSET_ID), None, None)]
fn merge_position_works_no_parent(
    collateral: Asset<MarketId>,
    expected_ct_001: Option<Asset<MarketId>>,
    expected_ct_110: Option<Asset<MarketId>>,
) {
    ExtBuilder::build().execute_with(|| {
        ...
        let ct_001 = Pallet::<Runtime>::position_from_parent_collection(
        ...
        .unwrap();
        let ct_110 = Pallet::<Runtime>::position_from_parent_collection(
            parent_collection_id,
            market_id,
            partition[1].clone(),
            Fuel::new(16, false),
        ...
        .unwrap();
        if let Some(expected) = expected_ct_001 {
            assert_eq!(ct_001, expected);
        }
...
fn merge_position_fails_on_insufficient_funds_foreign_token() {
    ExtBuilder::build().execute_with(|| {
        let alice = Account::new(0)
            .deposit(Asset::ForeignAsset(1), _99)
            .unwrap();
