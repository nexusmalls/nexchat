...
/// Bounded work budget for cryptographic collection-ID derivation.
/// 密码学集合 ID 派生使用的有界工作预算。
#[derive(
...
impl Fuel {
    /// Creates a work budget and selects whether all iterations must be consumed.
    /// 创建工作预算，并选择是否必须消耗全部迭代次数。
    pub fn new(total: u32, consume_all: bool) -> Self {
        Fuel { total, consume_all }
    }

    /// Returns whether collection-ID derivation must consume the entire budget.
    /// 返回集合 ID 派生是否必须消耗完整预算。
...
/// Standard alt-bn128-compatible combinatorial collection and position ID manager.
/// 标准的、兼容 alt-bn128 的组合集合与头寸 ID 管理器。
