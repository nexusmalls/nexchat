...
mod tests {
    ...
    // Nexus intentionally widens ForeignAsset from u32 to u64; these vectors lock the resulting
    // eight-byte little-endian payload. Nexus 有意将 ForeignAsset 从 u32 扩为 u64；这些向量
    // 锁定扩展后的八字节小端 payload。
    #[test_case(Asset::Ztg, vec![4])]
    #[test_case(Asset::ForeignAsset(0), vec![5, 0, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(1), vec![5, 1, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(2), vec![5, 2, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(3), vec![5, 3, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(4), vec![5, 4, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(5), vec![5, 5, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(6), vec![5, 6, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(7), vec![5, 7, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(8), vec![5, 8, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(9), vec![5, 9, 0, 0, 0, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(u32::MAX as u64 - 1), vec![5, 254, 255, 255, 255, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(u32::MAX as u64), vec![5, 255, 255, 255, 255, 0, 0, 0, 0])]
    #[test_case(Asset::ForeignAsset(u64::MAX), vec![5, 255, 255, 255, 255, 255, 255, 255, 255])]
