...
mod benchmarks {
    ...
    fn split_position_vertical_sans_parent(n: Linear<2, 32>, m: Linear<32, 64>) {
        ...
        let assets_out: Vec<_> = collection_ids
            ...
            .collect();
        let expected_event =
            <T as frame_system::Config>::RuntimeEvent::from(Event::<T>::TokenSplit {
                who: alice,
                parent_collection_id,
                market_id,
                partition,
        ...
        System::<T>::assert_last_event(expected_event);
    }
    ...
    fn merge_position_vertical_sans_parent(n: Linear<2, 32>, m: Linear<32, 64>) {
        ...
        let expected_event =
            <T as frame_system::Config>::RuntimeEvent::from(Event::<T>::TokenMerged {
                who: alice,
                parent_collection_id,
                market_id,
                partition,
    ...
    fn redeem_position_sans_parent(n: Linear<2, 32>, m: Linear<32, 64>) {
        ...
        *index_set.last_mut().unwrap() = false;
        ...
        let expected_event =
            <T as frame_system::Config>::RuntimeEvent::from(Event::<T>::TokenRedeemed {
                who: alice,
                parent_collection_id,
                market_id,
                index_set,
