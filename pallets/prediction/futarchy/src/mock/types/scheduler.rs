...
impl MockScheduler {
    ...
    pub fn called_once_with(
        ...
        let args =
            SCHEDULER_CALL_DATA.with(|value| value.borrow().first().expect("nonempty").clone());
...
thread_local! {
    static SCHEDULER_CALL_DATA: RefCell<Vec<SchedulerCallData>> =
        const { RefCell::new(vec![]) };
    static SCHEDULER_RETURN_VALUE: RefCell<DispatchResult> = const { RefCell::new(Ok(())) };
}
