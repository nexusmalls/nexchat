...
impl<T: Config> ProposalStorage<T> for Pallet<T> {
    fn count() -> u32 {
        ProposalCount::<T>::get()
    }
    ...
    fn add(block_number: BlockNumberFor<T>, proposal: Proposal<T>) -> Result<(), DispatchError> {
        let proposal_count = ProposalCount::<T>::get();
            ...
            proposal_count < T::MaxProposals::get(),
            Error::<T>::CacheFull
        );
    ...
    fn take(block_number: BlockNumberFor<T>) -> Result<ProposalsOf<T>, DispatchError> {
        ...
        let proposal_count = ProposalCount::<T>::get();
        let proposals_len: u32 = proposals
            .len()
            .try_into()
            .map_err(|_| Error::<T>::CacheFull)?;
    ...
    {
        let keys: Vec<_> = Proposals::<T>::iter_keys()
            .take(T::MaxProposals::get().saturated_into())
            .collect();
