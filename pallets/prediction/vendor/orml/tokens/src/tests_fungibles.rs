...
fn fungibles_unbalanced_trait_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            // set_balance
            assert_eq!(<Tokens as fungibles::Inspect<_>>::balance(DOT, &ALICE), 100);
            assert_ok!(<Tokens as fungibles::Unbalanced<_>>::write_balance(
                DOT, &ALICE, 10
            ));
            ...
            assert_eq!(
                <Tokens as fungibles::Inspect<_>>::total_balance(DOT, &ALICE),
                0
            );
            // set reserved
            assert_ok!(<Tokens as fungibles::Unbalanced<_>>::write_balance(
...
fn fungibles_mutate_hold_trait_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(<Tokens as fungibles::MutateHold<_>>::hold(
                DOT, REASON, &ALICE, 100
            ));
            assert_eq!(
                <Tokens as fungibles::InspectHold<_>>::balance_on_hold(DOT, REASON, &ALICE),
