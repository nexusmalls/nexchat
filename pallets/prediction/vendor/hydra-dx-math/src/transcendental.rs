    ...
    D::Bits: From<u32>,
{
    ...
    match (
        boundary.checked_div_int(n.into()),
        S::one().checked_sub(operand),
    ) {
...
mod tests {
    ...
    fn saturating_powi_high_precision_works() {
        ...
        assert_eq!(
            saturating_powi_high_precision::<S, D>(two, 0),
            D::from_num(one)
        );
        assert_eq!(
            saturating_powi_high_precision::<S, D>(zero, 2),
            D::from_num(zero)
        );
        assert_eq!(
            saturating_powi_high_precision::<S, D>(two, 1),
            D::from_num(2)
        );
        assert_eq!(
            saturating_powi_high_precision::<S, D>(two, 2),
            D::from_num(4)
        );
        assert_eq!(
            saturating_powi_high_precision::<S, D>(two, 3),
            D::from_num(8)
        );
        assert_eq!(
        ...
        assert_eq!(
            saturating_powi_high_precision::<S, D>(S::from_num(8) / 10, 2),
            expected
        );
    }
    ...
    fn powi_near_one_works() {
        ...
        assert_eq!(
            powi_near_one(S::from_num(9) / 10, 2),
            Some(S::from_num(81) / 100)
        );
    }
