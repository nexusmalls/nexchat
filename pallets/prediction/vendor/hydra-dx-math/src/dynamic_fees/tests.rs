...
fn protocol_fee_should_decrease_when_out_is_greater_than_int() {
    ...
    let calculated_fee =
        recalculate_protocol_fee(volume, 985, previous_fee, last_block_diff, params);
    assert_eq!(calculated_fee, expected_fee);
}
...
fn protocol_fee_should_increase_when_in_is_greater_than_out() {
    ...
    let calculated_fee =
        recalculate_protocol_fee(volume, 1005, previous_fee, last_block_diff, params);
    assert_eq!(calculated_fee, expected_fee);
}
...
fn procotol_fee_should_clamp_to_min_fee() {
    ...
    let calculated_fee =
        recalculate_protocol_fee(volume, 85, previous_fee, last_block_diff, params);
    assert_eq!(calculated_fee, expected_fee);
}
...
fn protocol_fee_should_clamp_to_max_fee() {
    ...
    let calculated_fee =
        recalculate_protocol_fee(volume, 105, previous_fee, last_block_diff, params);
    assert_eq!(calculated_fee, expected_fee);
}
