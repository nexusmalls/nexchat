...
pub fn price_weighted_average(prev: EmaPrice, incoming: EmaPrice, weight: Fraction) -> EmaPrice {
    ...
    if incoming >= prev {
        rounding_add(
            prev,
            multiply(weight, saturating_sub(incoming, prev)),
            Rounding::Down,
        )
    } else {
        rounding_sub(
            prev,
            multiply(weight, saturating_sub(prev, incoming)),
            Rounding::Up,
        )
    }
}
...
pub(super) fn rounding_add(l: EmaPrice, (r_n, r_d): (U512, U512), rounding: Rounding) -> EmaPrice {
    ...
    let n = l_n
        .saturating_mul(r_d)
        .saturating_add(r_n.saturating_mul(l_d));
    // d = l.d * r.d
...
pub(super) fn rounding_sub(l: EmaPrice, (r_n, r_d): (U512, U512), rounding: Rounding) -> EmaPrice {
    ...
    let n = l_n
        .saturating_mul(r_d)
        .saturating_sub(r_n.saturating_mul(l_d));
    // d = l.d * r.d
    let d = l_d.saturating_mul(r_d);
    round_to_rational((n, d), rounding)
}
