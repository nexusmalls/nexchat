...
pub(crate) fn round(r: &mut Rational) {
    r.mutate_numer_denom(|n, d| {
        ...
        if n_digits > 256 || d_digits > 256 {
            let shift = n_digits
                .saturating_sub(256)
                .max(d_digits.saturating_sub(256));
            n.shr_assign(shift);
            d.shr_assign(shift);
        }
    });
...
/// zero).
pub fn precise_balance_weighted_average(
    prev: Balance,
    incoming: Balance,
    weight: Rational,
) -> Integer {
...
pub fn naive_precise_price_ema(history: Vec<EmaPrice>, smoothing: Rational) -> Rational {
    ...
    for price in history.into_iter().skip(1) {
        current =
            precise_weighted_average(current.clone(), Rational::from(price), smoothing.clone());
    }
    current
}
...
fn precise_balance_ema_works() {
    ...
    let expected = {
        let res = ((Rational::from(history[0]) * 3 / 4 + history[1] / 4) * 3 / 4 + history[2] / 4)
            * 3
            / 4
            + history[3] / 4;
    ...
    let naive_ema =
        naive_precise_balance_ema(history.clone(), fraction_to_high_precision(smoothing));
