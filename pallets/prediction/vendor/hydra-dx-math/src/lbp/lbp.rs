...
pub fn calculate_out_given_in(
    ...
    let weight_ratio =
        div_to_fixed(in_weight.into(), out_weight.into(), Rounding::Down).ok_or(Overflow)?;
    ...
    out_reserve
        .checked_sub(new_out_reserve_calc)
        .ok_or(Overflow)
}
...
pub fn calculate_in_given_out(
    ...
    let weight_ratio =
        div_to_fixed(out_weight.into(), in_weight.into(), Rounding::Down).ok_or(Overflow)?;
...
pub fn calculate_linear_weights<
    BlockNumber: num_traits::CheckedSub + TryInto<u32> + TryInto<u128>,
>(
    start_x: BlockNumber,
    end_x: BlockNumber,
    start_y: LBPWeight,
