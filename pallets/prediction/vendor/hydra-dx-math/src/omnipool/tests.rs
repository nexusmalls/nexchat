...
use crate::omnipool::{
    calculate_add_liquidity_state_changes, calculate_buy_for_hub_asset_state_changes,
    calculate_buy_state_changes, calculate_cap_difference, calculate_fee_amount_for_buy,
    calculate_remove_liquidity_state_changes, calculate_sell_hub_state_changes,
    calculate_sell_state_changes, calculate_tvl_cap_difference, calculate_withdrawal_fee,
    verify_asset_cap,
};
...
fn calculate_buy_for_hub_asset_should_work_when_correct_input_provided() {
    ...
    let asset_fee = Permill::from_percent(0);
    let state_changes =
        calculate_buy_for_hub_asset_state_changes(&asset_state, amount_to_buy, asset_fee);
...
fn calculate_remove_liquidity_should_work_when_correct_input_provided() {
    ...
    let position = Position {
        ...
        shares: 3 * UNIT,
        price: (
            FixedU128::from_float(0.23).into_inner(),
...
fn calculate_remove_liquidity_should_work_when_current_price_is_smaller_than_position_price() {
    ...
    let position = Position {
        ...
        shares: 3 * UNIT,
        price: (
            FixedU128::from_float(2.23).into_inner(),
            1_000_000_000_000_000_000,
        ),
    };
...
fn calculate_cap_diff_should_work_correctly() {
    ...
    let result =
        calculate_cap_difference(&asset_state_2, 1_000_000_000_000_000_000, 5651225591124720);
    ...
    let result =
        calculate_cap_difference(&asset_state_2, 100_000_000_000_000_000, 5651225591124720);
    assert_eq!(result, Some(0));
}
...
fn calculate_tvl_cap_diff_should_work_correctly() {
    ...
    let result =
        calculate_tvl_cap_difference(&asset_state, &stable_asset, tvl_cap, total_hub_resrerve);
    assert_eq!(result, Some(0));
}
...
fn test_fee_amount() {
    assert_eq!(
        calculate_fee_amount_for_buy(Permill::from_float(0.01), 99),
        2
    );
    assert_eq!(
    ...
    assert_eq!(
        calculate_fee_amount_for_buy(Permill::from_percent(100), 99),
        99
    );
    assert_eq!(
        calculate_fee_amount_for_buy(Permill::from_percent(0), 99),
        0
    );
}
