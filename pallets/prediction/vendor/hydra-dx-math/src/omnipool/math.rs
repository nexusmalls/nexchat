...
use crate::omnipool::types::{
    AssetReserveState, AssetStateChange, HubTradeStateChange, LiquidityStateChange, Position,
    TradeFee, TradeStateChange,
};
...
pub fn calculate_sell_state_changes(
    ...
    let (in_hub_reserve, in_reserve, in_amount) =
        to_u256!(asset_in_state.hub_reserve, asset_in_state.reserve, amount);
...
pub fn calculate_sell_hub_state_changes(
    ...
    let (reserve_hp, hub_reserve_hp, amount_hp) = to_u256!(
        asset_out_state.reserve,
        asset_out_state.hub_reserve,
        hub_asset_amount
    );
...
pub fn calculate_buy_for_hub_asset_state_changes(
    ...
    let (hub_reserve_hp, amount_hp, hub_denominator_hp) = to_u256!(
        asset_out_state.hub_reserve,
        asset_out_amount,
        hub_denominator
    );
...
pub fn calculate_buy_state_changes(
    ...
    let (delta_hub_reserve_in_hp, in_hub_reserve_hp, in_reserve_hp) = to_u256!(
...
pub fn calculate_add_liquidity_state_changes(
    ...
    let (amount_hp, shares_hp, reserve_hp) =
        to_u256!(amount, asset_state.shares, asset_state.reserve);
...
pub fn calculate_remove_liquidity_state_changes(
    ...
    let p_x_r =
        U256::from(position_price.checked_mul_int(current_reserve)?).checked_add(U256::one())?;
...
pub fn calculate_spot_price(
    ...
    if let Some((protocol_fee, asset_fee)) = fee {
        ...
        let protocol_fee_multiplier = FixedU128::checked_from_rational(
            protocol_fee_multipiler.deconstruct() as u128,
            1_000_000,
        )?;

        let asset_fee_multiplier = Permill::from_percent(100).checked_sub(&asset_fee)?;
        let asset_fee_multiplier = FixedU128::checked_from_rational(
            asset_fee_multiplier.deconstruct() as u128,
            1_000_000,
        )?;
...
pub fn calculate_tvl_cap_difference(
    ...
    let max_hub_reserve = tvl
        .checked_mul(stable_hub_reserve)?
        .checked_div(stable_reserve)?;
...
pub(crate) fn calculate_burn_amount_based_on_fee_taken(
    ...
    let (taken_fee_hp, total_fee_amount_hp, extra_hp) =
        to_u256!(taken_fee, total_fee_amount, extra_hub_amount);
    // taken / fee  = X / extra_hp
    // X = taken * extra_hp / fee
    let hub_to_burn = taken_fee_hp
        .saturating_mul(extra_hp)
        .div(total_fee_amount_hp);
