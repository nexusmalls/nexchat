...
where
    Balance:
        Into<<FixedU128 as FixedPointNumber>::Inner> + Copy + CheckedAdd + CheckedSub + Default,
{
...
impl<Balance: CheckedAdd + CheckedSub + PartialOrd + Copy + Default + Saturating + Copy>
...
impl<Balance: CheckedAdd + CheckedSub + PartialOrd + Copy + Default> CheckedAdd
    for BalanceUpdate<Balance>
{
...
impl<
        Balance: Into<<FixedU128 as FixedPointNumber>::Inner> + CheckedAdd + CheckedSub + Copy + Default,
    > Add<Balance> for BalanceUpdate<Balance>
{
    ...
    > AssetStateChange<Balance>
{
    pub fn total_delta_hub_reserve(&self) -> BalanceUpdate<Balance> {
        self.delta_hub_reserve
            .saturating_merge(self.extra_hub_reserve_amount)
    }
...
impl TradeStateChange<crate::types::Balance> {
    pub fn account_for_fee_taken(self, taken_fee: crate::types::Balance) -> Self {
        let mut v = self;
        let extra_amt = calculate_burn_amount_based_on_fee_taken(
            taken_fee,
            v.fee.asset_fee,
            *v.asset_out.extra_hub_reserve_amount,
        );
...
impl HubTradeStateChange<crate::types::Balance> {
    pub fn account_for_fee_taken(self, taken_fee: crate::types::Balance) -> Self {
        ...
        let extra_amt = calculate_burn_amount_based_on_fee_taken(
            taken_fee,
            v.fee.asset_fee,
            *v.asset.extra_hub_reserve_amount,
        );
...
mod tests {
    ...
    fn balance_update_add(
        x: BalanceUpdate<u32>,
        y: BalanceUpdate<u32>,
        result: BalanceUpdate<u32>,
    ) {
    ...
    fn balance_update_checked_add(
        x: BalanceUpdate<u32>,
        y: BalanceUpdate<u32>,
        result: Option<BalanceUpdate<u32>>,
    ) {
    ...
    fn balance_update_saturating_add(
        x: BalanceUpdate<u32>,
        y: BalanceUpdate<u32>,
        result: BalanceUpdate<u32>,
    ) {
