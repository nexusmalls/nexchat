...
pub fn calculate_deferred_duration(
    ...
    let deferred_duration = defer_duration
        .saturating_mul(total_accumulated.saturating_sub(rate_limit))
        / rate_limit.max(1);
...
pub fn decay_accumulated_amount(
    ...
    accumulated_amount.saturating_sub(
        rate_limit.saturating_mul(blocks_since_last_update.saturated_into()) / defer_duration,
    )
}
