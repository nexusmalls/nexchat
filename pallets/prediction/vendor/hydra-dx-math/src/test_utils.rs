...
fn fraction_to_high_precision_works() {
    assert_eq!(
        fraction_to_high_precision(fraction::frac(1, 4)),
        Rational::from((1, 4))
    );
}
...
pub fn bigger_and_smaller_rational(
    min: u128,
    max: u128,
) -> impl Strategy<Value = ((u128, u128), (u128, u128))> {
    ((min + 1)..max, (min.max(1))..(max - 1)).prop_perturb(move |(a, b), mut rng| {
        ((a, b), (rng.gen_range(min..a), rng.gen_range(b..max)))
    })
}
