...
fn calculate_loyalty_multiplier_should_work() {
    ...
    ) in testing_values.iter()
    {
        ...
        assert_eq!(
            calculate_loyalty_multiplier(
                *periods,
                FixedU128::from_inner(123_580_000_000_000_000),
                23
            )
            .unwrap(),
            *expected_multiplier_3,
...
fn calculate_accumulated_rps_should_work() {
    ...
    for (accumulated_rps_now, total_shares, reward, expected_accumulated_rps) in
        testing_values.iter()
    {
        assert_approx_eq!(
...
fn calculate_valued_shares_should_work() {
    ...
    assert_eq!(
        calculate_valued_shares(16_874, 49_898_646).unwrap(),
        841_989_752_604
    );

    assert_eq!(
        calculate_valued_shares(16_874_138_468_415_354, 0).unwrap(),
        0
    );
...
fn calculate_global_farm_shares_should_work() {
    assert_eq!(
        calculate_global_farm_shares(0, FixedU128::from(0)).unwrap(),
        0
    );

    assert_eq!(
        calculate_global_farm_shares(16_841_351, FixedU128::from(0)).unwrap(),
        0
    );

    assert_eq!(
        calculate_global_farm_shares(16_841_351, FixedU128::from_inner(156_874_561_300_000_000))
            .unwrap(),
        2_641_979
    );

    assert_eq!(
        calculate_global_farm_shares(
            16_841_351,
            FixedU128::from_inner(18_641_535_156_874_561_300_000_000)
        )
        .unwrap(),
        313_948_636_755_764
    );
...
fn calculate_reward_should_work() {
    ...
    assert_eq!(
        calculate_reward(
            FixedU128::from(684_131),
            FixedU128::from(19_874_646),
            9_798_646
        )
        .unwrap(),
        188_041_063_042_690
