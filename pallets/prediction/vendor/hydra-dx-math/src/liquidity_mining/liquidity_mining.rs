...
pub fn calculate_loyalty_multiplier<
    Period: num_traits::CheckedSub + TryInto<u32> + TryInto<u128>,
>(
    periods: Period,
    ...
    let periods =
        FixedU128::from(TryInto::<u128>::try_into(periods).map_err(|_e| MathError::Overflow)?);
...
pub fn calculate_accumulated_rps(
    ...
    let rps =
        FixedU128::checked_from_rational(reward, total_shares).ok_or(MathError::DivisionByZero)?;

    rps.checked_add(&accumulated_rps_now)
        .ok_or(MathError::Overflow)
}
...
pub fn calculate_valued_shares(
    shares: Balance,
    incentivized_asset_balance: Balance,
...
pub fn calculate_yield_farm_rewards(
    ...
    let stake_in_global_farm = calculate_global_farm_shares(total_valued_shares, multiplier)
        .map_err(|_| MathError::Overflow)?;

    let yield_farm_rewards =
        calculate_reward(yield_farm_rpz, global_farm_rpz, stake_in_global_farm)
            .map_err(|_| MathError::Overflow)?;

    let delta_rpvs = FixedU128::checked_from_rational(yield_farm_rewards, total_valued_shares)
        .ok_or(MathError::Overflow)?;
...
>(
    ...
    let periods =
        TryInto::<u128>::try_into(periods_since_last_update).map_err(|_e| MathError::Overflow)?;
    ...
    let max_reward_for_periods = max_reward_per_period
        .checked_mul(periods)
        .ok_or(MathError::Overflow)?;
