...
fn calculate_accumulated_rps_should_work_when_total_stake_is_not_zero() {
    ...
    assert_eq!(
        calculate_accumulated_rps(rps_now, 0, 534_232 * ONE).unwrap(),
        rps_now
    );
...
fn sigmoid_should_work() {
    ...
    assert_eq!(
        sigmoid(1, a, b).unwrap(),
        FixedU128::from_inner(2_047_999_995_u128)
    );
    ...
    assert_eq!(
        sigmoid(1, a, b).unwrap(),
        FixedU128::from_inner(418_228_051_u128)
    );
...
fn calculate_percentage_amount_should_work() {
    ...
    assert_eq!(
        calculate_percentage_amount(3_000_000_u128, FixedU128::from(0)),
        0_u128
    );
