...
use num_traits::{
    CheckedDiv, CheckedMul, CheckedSub, One, SaturatingAdd, SaturatingMul, SaturatingSub, Zero,
};
...
pub fn calculate_out_given_in<const D: u8, const Y: u8>(
    ...
    let new_reserve_out =
        calculate_y_given_in::<D, Y>(amount_in, idx_in, idx_out, &reserves, amplification, pegs)?;
...
pub fn calculate_in_given_out<const D: u8, const Y: u8>(
    ...
    let new_reserve_in =
        calculate_y_given_out::<D, Y>(amount_out, idx_in, idx_out, &reserves, amplification, pegs)?;
...
pub fn calculate_out_given_in_with_fee<const D: u8, const Y: u8>(
    ...
    let amount_out = calculate_out_given_in::<D, Y>(
        initial_reserves,
        idx_in,
        idx_out,
        amount_in,
...
pub fn calculate_shares<const D: u8>(
    ...
    if share_issuance == 0 {
        ...
        let (issuance_hp, d_diff, d0) = to_u256!(
            share_issuance,
            adjusted_d.checked_sub(initial_d)?,
            initial_d
        );
...
pub(crate) fn calculate_d_internal<const D: u8>(
    ...
    for _ in 0..D {
        let d_p = xp_hp.iter().try_fold(d, |acc, v| {
            acc.checked_mul(d)?.checked_div((*v).checked_mul(n_coins)?)
        })?;
        let d_prev = d;
...
fn calculate_y_internal<const D: u8>(
    ...
    let (d_hp, n_coins_hp, ann_hp, precision_hp) =
        to_u256!(d, xp_hp.len().checked_add(1)?, ann, PRECISION as u128);
    ...
    for reserve in xp_hp.iter() {
        c = c
            .checked_mul(d_hp)?
            .checked_div((*reserve).checked_mul(n_coins_hp)?)?;
    }

    c = c
        .checked_mul(d_hp)?
        .checked_div(ann_hp.checked_mul(n_coins_hp)?)?;
...
fn peg_reserves(reserves: &[Balance], pegs: &[(Balance, Balance)]) -> Option<Vec<Balance>> {
    reserves.iter().zip(pegs.iter()).try_fold(
        Vec::with_capacity(reserves.len()),
        |mut acc, (v, peg)| {
            if let Some(result) =
                multiply_by_rational_with_rounding(*v, peg.0, peg.1, PTRounding::Down)
            {
                acc.push(result);
                Some(acc)
            } else {
...
pub fn calculate_share_prices<const D: u8>(
    ...
    for idx in 0..n {
        let price =
            calculate_share_price::<D>(reserves, amplification, issuance, idx, Some(d), pegs)?;
        r.push(price);
    }
    Some(r)
}
...
pub fn calculate_share_price<const D: u8>(
    ...
    let (d, c, xi, n, ann, issuance) =
        to_u256!(d, c, n_reserves[asset_idx], n_coins as u128, ann, issuance);

    let xann = xi.checked_mul(ann)?;
    let p1 = d.checked_mul(xann)?;
    let p2 = xi
        .checked_mul(c)?
        .checked_mul(n.saturating_add(U256::one()))?;
    ...
    let p_diff =
        U256::from(10u128.saturating_pow(18u8.saturating_sub(reserves[asset_idx].decimals) as u32));
...
pub fn calculate_spot_price(
    ...
    match (asset_in == pool_id, asset_out == pool_id) {
        ...
        (STABLE_ASSET, SHARE_ASSET) => {
            ...
            let update_reserves: &Vec<AssetReserve> = &updated_reserves
                .into_iter()
                .map(|(_, v)| v)
                .collect::<Vec<_>>();
...
pub fn calculate_spot_price_between_two_stable_assets(
    ...
    let c = reserves_hp.iter().try_fold(d, |acc, val| {
        acc.checked_mul(d)?.checked_div((*val).checked_mul(n)?)
    })?;
