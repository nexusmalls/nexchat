...
fn test_compare_precision_results_01() {
    ...
    let d_before =
        calculate_d::<D_ITERATIONS>(&balances, amp, &default_pegs(balances.len())).unwrap();
    let result = calculate_out_given_in_with_fee::<D_ITERATIONS, Y_ITERATIONS>(
        &balances,
        1,
        2,
    ...
    let updated_reserves = [
        balances[0],
        AssetReserve::new(
            balances[1].amount + 1_000_000_000_000_000_000,
            balances[1].decimals,
        ),
        AssetReserve::new(balances[2].amount - result.unwrap().0, balances[2].decimals),
    ];
    let d_after = calculate_d::<D_ITERATIONS>(
        &updated_reserves,
        amp,
        &default_pegs(updated_reserves.len()),
    )
    .unwrap();
    ...
    let updated_reserves = [
        ...
        AssetReserve::new(
            balances[2].amount + 1_000_000_000_000_000_000,
            balances[2].decimals,
        ),
    ];
    let d_after = calculate_d::<D_ITERATIONS>(
...
fn test_compare_precision_results_03() {
    ...
    let d_after =
        calculate_d::<D_ITERATIONS>(&updated_reserves, amp, &default_pegs(balances.len())).unwrap();
    assert!(d_after >= d_before);
    assert_eq!(result.unwrap(), (1000718003812077928, 0));
    ...
    let updated_reserves = [
        ...
        AssetReserve::new(
            balances[2].amount - 1000718003812077928,
            balances[2].decimals,
        ),
    ];
...
fn share_price_calculation_should_work_with_different_decimals() {
    ...
    let result = calculate_share_price::<D_ITERATIONS>(
        &balances,
        amp,
        issuance,
        0,
    ...
    assert_eq!(
        result,
        Some((
            74487238136284601991455700,
            171320935829579349745322922049517444521
        ))
    );
}
...
fn share_price_calculation_should_work_with_6_decimals() {
    ...
    assert_eq!(
        result,
        Some((
            279206572581786940496760242,
            279158579738033226972960348441675415837
        ))
    );
}
