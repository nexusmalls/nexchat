...
fn test_y_given_in() {
    ...
    assert_eq!(
        calculate_y_given_in::<D_ITERATIONS, Y_ITERATIONS>(
            amount_in,
            0,
            1,
            &reserves,
...
fn test_y_given_out() {
    ...
    assert_eq!(
        calculate_y_given_out::<D_ITERATIONS, Y_ITERATIONS>(
            amount_out,
            0,
            1,
            &reserves,
    ...
    assert_eq!(
        calculate_d_internal::<D_ITERATIONS>(
            &[1000u128 + expected_in, 2000u128 - amount_out],
            1,
            &default_pegs(2)
        ),
        Some(2918u128)
...
fn test_d_case() {
    ...
    let result = calculate_d_internal::<D_ITERATIONS>(
        &[500000000000008580273458u128, 10u128],
        amp,
        &default_pegs(2),
    );
...
fn test_d_case2() {
    ...
    let result = calculate_d_internal::<D_ITERATIONS>(
        &[500000000000000000000010u128, 11u128],
        amp,
        &default_pegs(2),
    );
...
fn test_shares() {
    ...
    let updated_reserves = &[
        AssetReserve::new(1000 * ONE, 12),
        AssetReserve::new(500, 12),
    ];
