...
use frame_support::{assert_err, assert_noop, assert_ok, traits::tokens::ExistenceRequirement};
        ...
        match (&$left, &$right, &$precision) {
            (left_val, right_val, precision_val) => {
                ...
                if diff > $precision {
                    panic!(
                        "{} is not {}-close to {}",
                        *left_val, *precision_val, *right_val
                    );
                }
            }
        }
...
fn single_asset_operations_and_swaps_fail_on_invalid_status_before_clean(status: PoolStatus) {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(Currencies::deposit(
            Swaps::pool_shares_id(DEFAULT_POOL_ID),
            &ALICE,
            _25
        ));
...
fn get_spot_price_returns_correct_results_cpmm(
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(Currencies::deposit(
            ASSET_A,
            &pool_account,
            balance_in - amount_in_pool
        ));
        assert_ok!(Currencies::deposit(
            ASSET_B,
            &pool_account,
            balance_out - amount_in_pool
        ));
...
fn pool_exit_decreases_correct_pool_parameters_with_exit_fee() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            Currencies::free_balance(pool_shares_id, &BOB),
            DEFAULT_LIQUIDITY - _10
        );
        assert_eq!(
            Currencies::free_balance(ASSET_A, &pool_account),
            DEFAULT_LIQUIDITY - _9
        );
        assert_eq!(
            Currencies::free_balance(ASSET_B, &pool_account),
            DEFAULT_LIQUIDITY - _9
        ...
        assert_eq!(
            Currencies::free_balance(ASSET_C, &pool_account),
            DEFAULT_LIQUIDITY - _9
        );
        assert_eq!(
            Currencies::free_balance(ASSET_D, &pool_account),
            DEFAULT_LIQUIDITY - _9
        );
        assert_eq!(
            Currencies::total_issuance(pool_shares_id),
            DEFAULT_LIQUIDITY - _10
        );
...
fn pool_exit_with_exact_pool_amount_exchanges_correct_values(
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_all_parameters(
            [
                _25 - asset_amount_joined + asset_amount_expected,
                _25,
                _25,
                _25,
            ],
...
fn create_pool_fails_on_too_many_assets() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let assets: Vec<Asset<MarketId>> = (0..=length)
            .map(|x| Asset::CategoricalOutcome(0, x))
            .collect::<Vec<_>>();
...
fn pool_exit_with_exact_pool_amount_fails_if_liquidity_drops_too_low() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        Currencies::slash(
            pool_shares_id,
            &BOB,
            _100 - Swaps::min_balance(pool_shares_id),
        );
...
fn pool_exit_burns_small_amounts() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        // below ED.
        let pool_id = 0;
        let assets = vec![Asset::CategoricalOutcome(0, 3), Asset::Ztg]
            .try_into()
            .unwrap();
        ...
        assert_eq!(
            Currencies::free_balance(Asset::CategoricalOutcome(0, 3), &ALICE),
            0
        );
    });
}
