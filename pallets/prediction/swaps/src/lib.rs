...
//! Constant-product weighted swap pools and their on-chain lifecycle.
//! 恒定乘积加权兑换池及其链上生命周期。
...
mod pallet {
    ...
    use frame_support::{
        ...
        traits::{tokens::ExistenceRequirement, Get, StorageVersion},
        transactional,
        weights::Weight,
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// 参见 [`zeitgeist_primitives::traits::Swaps::pool_exit_with_exact_asset_amount`]。
        ///
        /// # Weight
        ///
        /// Complexity: `O(1)`
        ...
        /// 参见 [`zeitgeist_primitives::traits::Swaps::pool_join_with_exact_asset_amount`]。
        ///
        /// # Weight
        ///
        /// Complexity: O(1)
        ...
        /// 参见 [`zeitgeist_primitives::traits::Swaps::swap_exact_amount_in`]。
        ///
        /// # Weight
        ///
        /// Complexity: `O(1)`
        ...
        /// 参见 [`zeitgeist_primitives::traits::Swaps::swap_exact_amount_out`]。
        ///
        /// # Weight
        ///
        /// Complexity: `O(1)`
        ...
        /// 强制退出指定流动性提供者的池份额，参数与普通退出一致但由 `who` 指定账户。
        ///
    ...
    #[pallet::config]
    /// Runtime configuration for weighted constant-product swap pools.
    /// 加权恒定乘积兑换池的 runtime 配置。
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        /// Asset identifier used by pool balances and pool-share tokens.
        /// 池余额与池份额代币使用的资产标识。
        ...
        /// Multi-asset currency implementation that custody and transfers pool funds.
        /// 托管并转移池资金的多资产货币实现。
        type MultiCurrency: MultiCurrency<Self::AccountId, CurrencyId = Self::Asset>;

        /// Weight provider for pallet calls and lifecycle operations.
        /// pallet 调用与生命周期操作的权重提供器。
        ...
        /// Pallet identifier used to derive deterministic pool accounts.
        /// 用于派生确定性池账户的 pallet 标识。
        #[pallet::constant]
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_swap_exact_amount_out(
            ...
            let pool_account_id = Pallet::<T>::pool_account_id(&pool_id);
                ...
                max_asset_amount_in.is_some() || max_price.is_some(),
                Error::<T>::LimitMissing
            );
            Self::ensure_minimum_balance(pool_id, &pool, asset_out, asset_amount_out)?;
        ...
        /// Calculates the current pool spot price between two assets.
        /// 计算池中两项资产之间的当前现货价格。
        pub fn get_spot_price(
            ...
            let pool = Self::pool_by_id(*pool_id)?;
                ...
                pool.assets.binary_search(asset_in).is_ok(),
                Error::<T>::AssetNotInPool
            );
            ensure!(
                pool.assets.binary_search(asset_out).is_ok(),
                Error::<T>::AssetNotInPool
            );
        ...
        #[inline]
        /// Derives the deterministic custody account for a pool.
        /// 派生池的确定性托管账户。
        ...
        pub(crate) fn burn_pool_shares(
            ...
            T::MultiCurrency::withdraw(shares_id, from, amount, ExistenceRequirement::AllowDeath)?;
            Ok(())
        }
        ...
        /// Loads a pool by identifier or returns `PoolDoesNotExist`.
        /// 按标识读取池，不存在时返回 `PoolDoesNotExist`。
        ...
        T: Config,
    {
        ...
        fn create_pool(
            ...
            weights: Vec<BalanceOf<T>>,
                ...
                assets.len() <= usize::from(T::MaxAssets::get()),
                Error::<T>::TooManyAssets
            );
            ensure!(
                assets.len() >= usize::from(T::MinAssets::get()),
                Error::<T>::TooFewAssets
            );
