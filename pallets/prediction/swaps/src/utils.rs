...
use alloc::vec::Vec;
use frame_support::{dispatch::DispatchResult, ensure, traits::tokens::ExistenceRequirement};
    ...
    T: crate::Config,
{
    Pallet::<T>::ensure_pool_is_active(p.pool)?;
        ...
        p.pool.assets.binary_search(&p.asset_in).is_ok(),
        Error::<T>::AssetNotInPool
    );
    ensure!(
        p.pool.assets.binary_search(&p.asset_out).is_ok(),
        Error::<T>::AssetNotInPool
    );
