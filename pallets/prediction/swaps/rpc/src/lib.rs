...
//! Host-side JSON-RPC facade for the swaps runtime API.
//! swaps runtime API 的宿主侧 JSON-RPC 门面。
...
#[rpc(client, server)]
/// JSON-RPC methods for querying weighted swap pools.
/// 查询加权兑换池的 JSON-RPC 方法。
...
/// A host-side implementation of [`SwapsApi`].
/// [`SwapsApi`] 的宿主侧实现。
...
impl<C, B> Swaps<C, B> {
    /// Creates a swaps RPC handler backed by the supplied client.
    /// 使用给定客户端创建 swaps RPC 处理器。
...
/// Errors returned while executing swaps runtime API calls.
/// 执行 swaps runtime API 调用时返回的错误。
pub enum Error {
