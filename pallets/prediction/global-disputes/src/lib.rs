...
/// Possession information specialized for this runtime.
///
/// 针对此运行时具体化的所有权信息。
pub type PossessionOf<T> = crate::types::Possession<AccountIdOf<T>, BalanceOf<T>, OwnerInfoOf<T>>;
/// Initial dispute item specialized for this runtime.
///
/// 针对此运行时具体化的初始争议项。
...
mod pallet {
    ...
    use frame_support::{
        ...
        traits::{
            Currency, ExistenceRequirement, Get, LockIdentifier, LockableCurrency, StorageVersion,
            WithdrawReasons,
        },
    ...
    /// Per-account market vote locks specialized for this runtime.
    ///
    /// 针对此运行时具体化的账户市场投票锁定信息。
    ...
    #[pallet::config]
    /// Runtime configuration for global disputes.
    ///
    /// 全局争议的运行时配置。
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Dispute scheduler and resolver used by this pallet.
        ///
        /// 本 pallet 使用的争议调度与解决接口。
        ...
        ///
        /// 私有 API `push_vote_outcome` 中单个投票结果允许的最大所有者数量。
        #[pallet::constant]
        ...
        /// Weight implementation for pallet calls.
        ///
        /// Pallet 调用的权重实现。
    ...
    impl<T: Config> Pallet<T> {
        ...
        ///
        /// 支付固定费用，为全局争议增加投票结果；结果已存在、争议未开始或已结束时失败。
        ///
        /// # Arguments
        ///
        ...
        pub fn refund_vote_fees(
            ...
            let gd_info =
                ...
                gd_info.status == GdStatus::Destroyed,
                Error::<T>::GlobalDisputeNotDestroyed
            );
            ...
            Ok((Some(T::WeightInfo::refund_vote_fees(
                removed_keys_amount,
                owners_len,
            )))
            .into())
        ...
        ///
        /// 清理全部结果，使获胜结果的所有者能够领取奖励；争议尚未结束时失败。
        ///
        /// # Arguments
        ///
        /// - `market_id`: The id of the market.
        ...
        pub fn purge_outcomes(
            ...
            let mut gd_info =
                ...
                Error::<T>::UnfinishedGlobalDispute
            );
            ...
            // weight for max owners, because we don't know
            Ok((Some(T::WeightInfo::purge_outcomes(
                removed_keys_amount,
                owners_len,
            )))
            .into())
        ...
        pub fn reward_outcome_owner(
            ...
            let reward_account_free_balance = T::Currency::free_balance(&reward_account);
                ...
                !reward_account_free_balance.is_zero(),
                Error::<T>::NoFundsToReward
            );
        ...
        pub fn vote_on_outcome(
            ...
            let voter_free_balance = T::Currency::free_balance(&voter);
                ...
                amount >= T::MinOutcomeVoteAmount::get(),
            ...
            } = gd_info.status
            {
                    ...
                    add_outcome_end < now && now <= vote_end,
                    Error::<T>::NotInGdVotingPeriod
                );
            } else {
                return Err(Error::<T>::InvalidGlobalDisputeStatus.into());
            }
            ...
            let lock_amount = match lock_info.binary_search_by_key(&market_id, |i| i.0) {
                Ok(i) => {
                    ...
                    let new_amount_acc = amount.saturating_add(prev_amount_acc);
                        ...
                        new_amount_acc <= voter_free_balance,
            ...
            Ok(Some(T::WeightInfo::vote_on_outcome(
                outcome_owners_len,
                vote_lock_counter,
            ))
            .into())
        ...
        /// Fails if the global dispute is not concluded yet.
        ///
        /// 从已完成或已销毁的全局争议中解锁原生代币；争议尚未结束时保持锁定。
        ///
        /// # Arguments
        ///
    ...
    impl<T: Config> Pallet<T> {
        /// Derives the reward account for a market.
        ///
        /// 派生指定市场的奖励账户。
        ...
        fn reward_shared_possession(
            ...
            Ok((Some(T::WeightInfo::reward_outcome_owner_shared_possession(
                owners_len,
            )))
            .into())
        }
