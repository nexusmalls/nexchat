...
mod pallet {
    ...
    #[pallet::config]
    /// Runtime dependencies for order placement, fills, cancellation, fees, and market lookup.
    /// 订单创建、成交、撤销、费用分配与市场查询所需的 runtime 依赖。
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        /// Reservable multi-currency backend for collateral and outcome assets.
        /// 用于抵押资产与结果资产的可储备多币种后端。
        ...
        /// Fee distributor that charges only the market base asset.
        /// 仅从市场基础资产收取费用的分配器。
        ...
        #[pallet::constant]
        /// Stable pallet identifier used for named reserves.
        /// 用于命名储备的稳定 pallet 标识符。
    ...
    impl<T: Config> Pallet<T> {
        /// Removes an order and releases its remaining named reserve.
        /// 撤销订单并释放其剩余命名储备。
        ///
        /// # Weight
        ///
        ...
        /// 全部（`maker_partial_fill` = None）或部分成交既有订单。
        ///
        ...
        /// Place a new base-asset/outcome-asset order and reserve the maker funds.
        /// 创建基础资产与结果资产之间的订单，并储备 maker 资金。
        ///
        /// # Weight
        ///
        /// Complexity: `O(1)`
    ...
    impl<T: Config> Pallet<T> {
        /// Returns the orderbook's named-reserve identifier.
        /// 返回订单簿的命名储备标识符。
        #[inline]
        ...
        fn ensure_ratio_quotient_valid(order_data: &OrderOf<T>) -> DispatchResult {
            ...
            let is_ratio_quotient_valid = maker_full_fill.is_zero()
                ...
                is_ratio_quotient_valid,
                Error::<T>::PartialFillNearFullFillNotAllowed
            );
            Ok(())
        }
        ...
        fn do_remove_order(order_id: OrderId, who: AccountIdOf<T>) -> DispatchResult {
            ...
            Self::deposit_event(Event::OrderRemoved {
                order_id,
                maker: maker.clone(),
            });
        ...
        fn charge_external_fees(
            ...
            if maker_asset_is_base {
                ...
                Ok((
                    ...
                    ExternalFeeOf::<T> {
                        account: order_data.maker.clone(),
                        amount: fee_amount,
                    },
                ))
            }
        ...
        fn do_fill_order(
            ...
            let maker_fill = maker_partial_fill.unwrap_or(order_data.taker_amount);
                ...
                maker_fill <= order_data.taker_amount,
                Error::<T>::AmountTooHighForOrder
            );
        ...
        fn do_place_order(
            ...
            let outcome_asset = if maker_asset == base_asset {
                taker_asset
                    ...
                    Error::<T>::MarketBaseAssetNotPresent
                );
                maker_asset
            };
