...
where
    T: Config,
{
    /// Creates a scoreboard with absolute and relative price margins.
    /// 使用绝对与相对价格阈值创建计分板。
    pub fn new(
        start: BlockNumberFor<T>,
        victory_margin: u128,
    ...
    /// Records one price observation at the supplied block.
    /// 在指定区块记录一次价格观测。
    pub fn update(
        &mut self,
        ...
        let margin_rel = margin_abs
            .bdiv(negative_outcome_price)
            .unwrap_or(Zero::zero());
    ...
    /// Returns whether the positive outcome has achieved the required margin.
    /// 返回正向结果是否已达到要求的胜出分差。
    ...
    /// Advances the observation window without awarding either outcome a point.
    /// 推进观测窗口，但不向任一结果计分。
