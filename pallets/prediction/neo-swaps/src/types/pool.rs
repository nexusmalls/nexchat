...
/// Stored LMSR pool state, reserves, collateral, fees, and liquidity-share manager.
/// 已存储的 LMSR 池状态、储备、抵押资产、费用与流动性份额管理器。
    ...
    S: Get<u32>,
{
    ...
    fn calculate_numerical_threshold(&self) -> BalanceOf<T> {
        ...
        self.liquidity_parameter
            .saturating_mul(EXP_NUMERICAL_LIMIT.saturated_into())
    }
