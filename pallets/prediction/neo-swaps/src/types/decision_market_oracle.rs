...
where
    T: Config,
{
    /// Creates a decision-market oracle for two pool outcomes.
    /// 为池中的两个结果创建决策市场预言机。
    pub fn new(
        pool_id: T::PoolId,
        positive_outcome: AssetOf<T>,
    ...
    T: Config,
{
    ...
    fn evaluate(&self) -> (Weight, bool) {
        (
            T::WeightInfo::decision_market_oracle_evaluate(),
            self.scoreboard.evaluate(),
        )
    }
    ...
    fn update(&mut self, now: Self::BlockNumber) -> Weight {
        if let Ok((positive_outcome_price, negative_outcome_price)) = self.try_get_prices() {
            self.scoreboard
                .update(now, positive_outcome_price, negative_outcome_price);
        } else {
