...
mod benchmarks {
    ...
    fn join_leaf(n: Linear<2, 128>) {
        ...
        let max_node_count = LiquidityTreeOf::<T>::max_node_count();
        assert_eq!(
            pool.liquidity_shares_manager.node_count(),
            max_node_count - 1
        );
    ...
    fn combo_buy(n: Linear<1, 7>) {
        ...
        let sell_arg = (middle..asset_count)
            .map(|i| assets[i as usize])
            .collect::<Vec<_>>();
