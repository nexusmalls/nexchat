...
mod pallet {
    ...
    /// Runtime account identifier. / Runtime 账户标识。
    pub type AccountIdOf<T> = <T as frame_system::Config>::AccountId;
    /// Pool or market asset identifier. / 池或市场资产标识。
    pub type AssetOf<T> = Asset<MarketIdOf<T>>;
    /// Multi-currency balance type. / 多币种余额类型。
    pub type BalanceOf<T> =
        <<T as Config>::MultiCurrency as MultiCurrency<AccountIdOf<T>>>::Balance;
    /// Market identifier supplied by market commons. / 由市场公共模块提供的市场标识。
    pub type MarketIdOf<T> = <<T as Config>::MarketCommons as MarketCommonsPalletApi>::MarketId;
    /// Combinatorial cryptographic work budget. / 组合代币密码学工作预算。
    ...
    #[pallet::config]
    /// Runtime dependencies for LMSR pools, collateral transfers, and combinatorial operations.
    /// LMSR 池、抵押资产转账与组合操作所需的 runtime 依赖。
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Complete-set mint and redemption operations. / 完全集合的铸造与赎回操作。
        ...
        /// Existing market storage and lookup boundary. / 既有市场的存储与查询边界。
        ...
        /// ORML multi-currency backend for collateral and outcome assets.
        /// 用于抵押资产与结果资产的 ORML 多币种后端。
        type MultiCurrency: MultiCurrency<Self::AccountId, CurrencyId = AssetOf<Self>>;

        /// Identifier shared by pools and their source markets. / 池及其来源市场共用的标识。
        ...
        /// Generated weight provider. / 生成的权重提供器。
        ...
        /// 每个池允许的最大流动性树深度；最多支持 `2^(depth + 1) - 1` 个 LP，且必须小于 16。
        #[pallet::constant]
        ...
        /// Maximum configurable swap fee. / 可配置的最大兑换费率。
        #[pallet::constant]
        type MaxSwapFee: Get<BalanceOf<Self>>;

        /// Pallet account derivation identifier. / Pallet 账户派生标识。
        #[pallet::constant]
    ...
    /// State changes emitted by pool deployment, trading, and liquidity operations.
    /// 池部署、交易与流动性操作产生的状态变更事件。
    pub enum Event<T>
    where
        T: Config,
    {
    ...
    #[pallet::error]
    /// Business, validation, and numerical failures returned by Neo Swaps.
    /// Neo Swaps 返回的业务、校验与数值计算错误。
    pub enum Error<T> {
    ...
    #[derive(
        Decode, DecodeWithMemTracking, Encode, Eq, PartialEq, PalletError, RuntimeDebug, TypeInfo,
    )]
    /// Numerical safety-limit violations in LMSR calculations.
    /// LMSR 计算中的数值安全边界违规。
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn buy(
            ...
            let asset_count_real_u16: u16 = asset_count_real
                .try_into()
                .map_err(|_| Error::<T>::NarrowingConversion)?;
            ensure!(
                asset_count == asset_count_real_u16,
        ...
        pub fn join(
            ...
            let asset_count_real = pool.assets().len();
                ...
                max_amounts_in.len() == asset_count_real,
                Error::<T>::IncorrectVecLen
            );
        ...
        pub fn exit(
            ...
            let min_amounts_out_len = min_amounts_out.len();
                ...
                min_amounts_out_len == asset_count_real,
                Error::<T>::IncorrectVecLen
            );

            // Ensure that the conversion in the weight calculation doesn't saturate.
            let min_amounts_out_len_u32: u32 = min_amounts_out_len
                .try_into()
                .map_err(|_| Error::<T>::NarrowingConversion)?;
        ...
        pub fn deploy_pool(
            ...
            let spot_prices_len: u32 = spot_prices
                .len()
                .try_into()
                .map_err(|_| Error::<T>::NarrowingConversion)?;
            ensure!(
                spot_prices_len == asset_count_u32,
                Error::<T>::IncorrectVecLen
            );
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(crate) fn do_join(
            ...
            let weight = <Self as PoolStorage>::try_mutate_pool(&pool_id, |pool| {
                ...
                let asset_count_u32 = max_amounts_in
                    .len()
                    .try_into()
                    .map_err(|_| Error::<T>::NarrowingConversion)?;
                let ratio =
                ...
                let benchmark_info = pool
                    .liquidity_shares_manager
                    .join(&who, pool_shares_amount)?;
        ...
        pub(crate) fn do_deploy_pool(
            ...
            let asset_count_u16: u16 = spot_prices
                .len()
                .try_into()
                .map_err(|_| Error::<T>::NarrowingConversion)?;
            let asset_count_u32: u32 = asset_count_u16.into();
            ensure!(
                asset_count_u16 == market.outcomes(),
                Error::<T>::IncorrectVecLen
            );
            ensure!(
                asset_count_u32 <= MaxAssets::get(),
                Error::<T>::AssetCountAboveMax
            );
            ensure!(
                swap_fee >= MIN_SWAP_FEE.saturated_into(),
                Error::<T>::SwapFeeBelowMin
            );
            ensure!(
                swap_fee <= T::MaxSwapFee::get(),
                Error::<T>::SwapFeeAboveMax
            );
            ensure!(
                spot_prices
                    .iter()
                    .fold(Zero::zero(), |acc: BalanceOf<T>, &val| acc
                        .saturating_add(val))
                    == BASE.saturated_into(),
        ...
        pub(crate) fn do_deploy_combinatorial_pool(
            ...
            let (collection_ids, position_ids, collateral) =
                ...
                spot_prices.len() == collection_ids.len(),
                Error::<T>::IncorrectVecLen
            );
            ensure!(
                spot_prices
        ...
        pub(crate) fn do_combo_sell(
            ...
            <Self as PoolStorage>::try_mutate_pool(&pool_id, |pool| {
                ...
                let total_assets = buy
                    .len()
                    .saturating_add(keep.len())
                    .saturating_add(sell.len());
                ensure!(
        ...
        pub(crate) fn split_markets(
            ...
            for market in markets.iter() {
                ...
                total_splits <= T::MaxSplits::get(),
                Error::<T>::MaxSplitsExceeded
            );
        ...
        pub(crate) fn try_common_collateral(
            ...
            for market_id in market_ids.iter() {
                let market = T::MarketCommons::market(market_id)?;
                    ...
                    market.base_asset == collateral,
                    Error::<T>::CollateralMismatch
                );
            }
