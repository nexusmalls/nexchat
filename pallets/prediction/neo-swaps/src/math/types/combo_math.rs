...
mod detail {
    ...
    fn calculate_swap_amount_out_for_sell_fixed(
        ...
        let (amount_buy_keep, buy_keep) = if keep.is_empty() {
            ...
            let delta_keep = amount_buy
                .checked_sub(delta_buy)?
                .checked_sub(amount_keep)?;

            let buy_intermediate = buy
                .into_iter()
                .map(|x| x.checked_add(delta_buy))
                .collect::<Option<Vec<_>>>()?;
            let keep_intermediate = keep
                .into_iter()
                .map(|x| x.checked_sub(delta_keep))
                .collect::<Option<Vec<_>>>()?;
            let buy_keep = buy_intermediate
                .into_iter()
                .chain(keep_intermediate)
                .collect();
...
mod tests {
    ...
    fn calcuate_spot_price_works(
        ...
        assert_eq!(
            MockMath::calculate_spot_price(buy, sell, liquidity).unwrap(),
            expected
        );
    }
