...
mod detail {
    ...
    fn calculate_swap_amount_out_for_buy_fixed(
        ...
        let reserve_plus_blob = if ln_neg {
...
mod tests {
    ...
    fn calcuate_spot_price_works(
        ...
        assert_eq!(
            MockMath::calculate_spot_price(reserve, liquidity).unwrap(),
            expected
        );
    }
    ...
    fn exp_does_not_overflow_or_underflow(neg: bool, expected: FixedType) {
        let result: FixedType = exp(
            FixedType::checked_from_num(EXP_NUMERICAL_THRESHOLD).unwrap(),
            neg,
        )
        .unwrap();
