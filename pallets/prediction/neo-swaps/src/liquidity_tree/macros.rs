...
macro_rules! assert_liquidity_tree_state {
    (
        ...
        for index in 0..max_len {
            match ($expected_nodes.get(index), actual_nodes.get(index)) {
                ...
                (Some(exp), None) => {
                    error = true;
                    eprintln!(
                        "assert_liquidity_tree_state: Missing node at index {}",
                        index
                    );
                    eprintln!("    {:?}", exp);
                }
        ...
        assert_eq!(
            $expected_account_to_index,
            $tree.account_to_index.clone().into_inner()
        );
        assert_eq!(
            $expected_abandoned_nodes,
            $tree.abandoned_nodes.clone().into_inner()
        );
    };
}
