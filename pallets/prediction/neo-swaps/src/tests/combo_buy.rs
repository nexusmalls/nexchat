...
fn combo_buy_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let amount_in = amount_in_minus_fees
            .bdiv(_1 - total_fee_percentage)
            .unwrap(); // This is exactly _10 after deducting fees.
        ...
        let pool_outcomes_before: Vec<_> = pool
            .assets()
            .iter()
...
fn combo_buy_works_multi_market(
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        let buy: Vec<_> = buy_indices
            .iter()
            .map(|&i| pool.assets()[i as usize])
            .collect();
        let keep: Vec<_> = keep_indices
            .iter()
            .map(|&i| pool.assets()[i as usize])
            .collect();
        let sell: Vec<_> = sell_indices
            .iter()
            .map(|&i| pool.assets()[i as usize])
            .collect();
