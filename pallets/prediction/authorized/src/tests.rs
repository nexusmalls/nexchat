...
fn authorize_market_outcome_does_not_reset_dispute_resolution() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            ...
            AuthorityReport {
                outcome: OutcomeReport::Scalar(1),
                resolve_at: resolve_at_0
            }
        );

        assert_eq!(
            mock_storage::MarketIdsPerDisputeBlock::<Runtime>::get(resolve_at_0),
            vec![0]
        );
...
fn on_resolution_fails_if_no_report_was_submitted() {
    ExtBuilder::default().build().execute_with(|| {
        let report = Authorized::on_resolution(&0, &market_mock::<Runtime>())
            .unwrap()
            .result;
...
fn authorized_market_outcome_can_handle_multiple_markets() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            AuthorizedOutcomeReports::<Runtime>::get(0).unwrap(),
            AuthorityReport {
                outcome: OutcomeReport::Scalar(123),
                resolve_at
            }
        );
        assert_eq!(
            ...
            AuthorityReport {
                outcome: OutcomeReport::Scalar(456),
                resolve_at
            }
        );
    });
...
fn get_auto_resolve_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let resolve_at = now + <Runtime as crate::Config>::CorrectionPeriod::get();
        assert_eq!(
            Authorized::get_auto_resolve(&0, &market).result,
            Some(resolve_at)
        )
    });
}
