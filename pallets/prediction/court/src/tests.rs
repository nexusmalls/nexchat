...
fn join_court_saves_consumed_stake_and_active_lock_for_double_join() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            CourtPool::<Runtime>::get().into_inner()[0].consumed_stake,
            consumed_stake
        );
        assert_eq!(
            Participants::<Runtime>::get(ALICE).unwrap().active_lock,
            active_lock
        );
    });
}
...
fn prepare_exit_court_removes_middle_staked_juror() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(Court::prepare_exit_court(RuntimeOrigin::signed(
            middle_index
        )));
        assert_eq!(CourtPool::<Runtime>::get().into_inner().len(), len - 1);
        CourtPool::<Runtime>::get()
...
fn vote_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            old_draws[alice_index].slashable,
            new_draws[alice_index].slashable
        );
...
fn denounce_vote_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            old_draws[0].court_participant,
            new_draws[0].court_participant
        );
...
fn appeal_shifts_auto_resolve() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            MarketIdsPerDisputeBlock::<Runtime>::get(resolve_at_0),
            vec![0]
        );
        ...
        assert_eq!(
            MarketIdsPerDisputeBlock::<Runtime>::get(resolve_at_1),
            vec![0]
        );
...
fn appeal_draws_total_weight_is_correct() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            last_draws_total_weight,
            Court::necessary_draws_weight(0usize) as u32
        );
...
fn reassign_court_stakes_slashes_tardy_jurors_and_rewards_winners() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_ok!(Court::reassign_court_stakes(
        ...
        assert_eq!(
            free_alice_after,
            free_alice_before - old_draws[ALICE as usize].slashable
        );
        ...
        assert_eq!(
            free_bob_after,
            free_bob_before - old_draws[BOB as usize].slashable
        );
        ...
        assert_eq!(
            free_dave_after,
            free_dave_before - old_draws[DAVE as usize].slashable
        );
        ...
        assert_eq!(
            free_eve_after,
            free_eve_before - old_draws[EVE as usize].slashable
        );
    });
}
...
fn reassign_court_stakes_emits_event() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let market_id = <CourtIdToMarketId<Runtime>>::get(court_id).unwrap();
        let market = MarketCommons::market(&market_id).unwrap();
        let _ = Court::on_resolution(&market_id, &market)
            .unwrap()
            .result
            .unwrap();
...
fn reassign_court_stakes_updates_court_status() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let market_id = <CourtIdToMarketId<Runtime>>::get(court_id).unwrap();
        let market = MarketCommons::market(&market_id).unwrap();
        let resolution_outcome = Court::on_resolution(&market_id, &market)
            .unwrap()
            .result
            .unwrap();
...
fn reassign_court_stakes_decreases_active_lock() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert!(<Participants<Runtime>>::get(ALICE)
            .unwrap()
            .active_lock
            .is_zero());
        assert!(<Participants<Runtime>>::get(BOB)
            .unwrap()
            .active_lock
            .is_zero());
        assert!(<Participants<Runtime>>::get(CHARLIE)
            .unwrap()
            .active_lock
            .is_zero());
        assert!(<Participants<Runtime>>::get(DAVE)
            .unwrap()
            .active_lock
            .is_zero());
    });
...
fn reassign_court_stakes_slashes_loosers_and_awards_winners() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            free_charlie_after,
            free_charlie_before + charlie_share * slashed
        );
...
fn reassign_court_stakes_works_for_delegations() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let delegated_stakes_charlie: crate::DelegatedStakesOf<Runtime> = vec![
            (ALICE, 2 * MinJurorStake::get()),
            (BOB, MinJurorStake::get()),
        ]
        .try_into()
        .unwrap();

        let delegated_stakes_dave: crate::DelegatedStakesOf<Runtime> = vec![
            (ALICE, 2 * MinJurorStake::get()),
            (BOB, MinJurorStake::get()),
        ]
        .try_into()
        ...
        let draws: crate::SelectedDrawsOf<Runtime> = vec![
            ...
            Draw {
                ...
                weight: 1,
                vote: Vote::Delegated {
                    delegated_stakes: delegated_stakes_charlie.clone(),
                },
                slashable: charlie_slashable,
            },
            Draw {
                ...
                weight: 1,
                vote: Vote::Delegated {
                    delegated_stakes: delegated_stakes_dave.clone(),
                },
                slashable: dave_slashable,
            },
        ]
        ...
        let bob_slashed = last_draws
            .iter()
            .find(|draw| draw.court_participant == BOB)
            .unwrap()
            .slashable;
        let charlie_delegated_bob_slashed = delegated_stakes_charlie
            .iter()
            .find(|(acc, _)| *acc == BOB)
            .unwrap()
            .1;
        let dave_delegated_bob_slashed = delegated_stakes_dave
            .iter()
            .find(|(acc, _)| *acc == BOB)
            .unwrap()
            .1;
        ...
        let charlie_delegated_alice_slashable = delegated_stakes_charlie
            .iter()
            .find(|(acc, _)| *acc == ALICE)
            .unwrap()
            .1;
        let dave_delegated_alice_slashable = delegated_stakes_dave
            .iter()
            .find(|(acc, _)| *acc == ALICE)
            .unwrap()
            .1;
        ...
        assert_eq!(
            free_dave_after,
            free_dave_before + dave_rewarded - dave_delegated_bob_slashed
        );
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        let draws: crate::SelectedDrawsOf<Runtime> = vec![
            Draw {
                ...
                vote: Vote::Revealed {
                    commitment,
                    vote_item: wrong_vote_item_1.clone(),
                    salt,
                },
                slashable: MinJurorStake::get(),
            },
            Draw {
                ...
                vote: Vote::Revealed {
                    commitment,
                    vote_item: wrong_vote_item_0.clone(),
                    salt,
                },
                slashable: 2 * MinJurorStake::get(),
            },
...
fn on_resolution_sets_court_status() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            Court::on_resolution(&market_id, &market).unwrap().result,
            Some(ORACLE_REPORT)
        );
        let court = <Courts<Runtime>>::get(court_id).unwrap();
        assert_eq!(
            court.status,
            CourtStatus::Closed {
                winner: VoteItem::Outcome(ORACLE_REPORT)
            }
        );
    });
}
...
fn on_resolution_fails_if_court_not_found() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_noop!(
            Court::on_resolution(&market_id, &market),
            Error::<Runtime>::CourtNotFound
        );
    });
}
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_noop!(
            Court::exchange(
                ...
                &market,
                &ORACLE_REPORT,
                NegativeImbalance::<Runtime>::zero()
            ),
            Error::<Runtime>::MarketDoesNotHaveCourtMechanism
        );
    });
...
fn exchange_slashes_unjustified_and_unreserves_justified_appealers() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        while (number as usize) < MaxAppeals::get() as usize {
            ...
            let appealed_vote_item = if number.is_multiple_of(2) {
        ...
        {
            assert_eq!(
                Balances::reserved_balance_named(&Court::reserve_id(), &backer),
                0
            );
...
fn get_auto_resolve_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            Court::get_auto_resolve(&market_id, &market).result,
            Some(appeal_end)
        );
    });
}
...
#[test]
fn on_global_dispute_retains_court_for_final_appeal_bond_exchange() {
    ExtBuilder::default().build().execute_with(|| {
        let court_id = initialize_court();
        let market_id = <CourtIdToMarketId<Runtime>>::get(court_id).unwrap();
        let market = MarketCommons::market(&market_id).unwrap();
...
fn select_participants_updates_juror_consumed_stake() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let consumed_stake_before = jurors
            .iter()
            .map(|juror| juror.consumed_stake)
            .sum::<u128>();
        ...
        let jurors = CourtPool::<Runtime>::get();
        let consumed_stake_after = jurors
            .iter()
            .map(|juror| juror.consumed_stake)
            .sum::<u128>();
        assert_ne!(consumed_stake_before, consumed_stake_after);
        assert_eq!(
            consumed_stake_before + total_draw_slashable,
            consumed_stake_after
        );
    });
}
...
fn appeal_reduces_active_lock_from_old_draws() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        old_draws.iter().for_each(|draw| {
            let juror = draw.court_participant;
            let p_info = <Participants<Runtime>>::get(juror).unwrap();
            if let Some(new_draw) = new_draws
                .iter()
                .find(|new_draw| new_draw.court_participant == juror)
            {
...
fn on_dispute_creates_correct_court_info() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            round_ends.aggregation,
            round_ends.vote + AggregationPeriod::get()
        );
        assert_eq!(
            round_ends.appeal,
            round_ends.aggregation + AppealPeriod::get()
        );
...
fn get_winner_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            winner.into_outcome().unwrap(),
            OutcomeReport::Scalar(1002u128)
        );
...
fn random_jurors_returns_a_subset_of_jurors() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        for draw in random_jurors {
            assert!(jurors
                .iter()
                .any(|el| el.court_participant == draw.court_participant));
        }
    });
}
...
fn handle_inflation_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            free_balance_after_0 - free_balances_before[&jurors_list[0]],
            216_002
        );

        let free_balance_after_1 = Balances::free_balance(jurors_list[1]);
        assert_eq!(
            free_balance_after_1 - free_balances_before[&jurors_list[1]],
            2_160_021
        );

        let free_balance_after_2 = Balances::free_balance(jurors_list[2]);
        assert_eq!(
            free_balance_after_2 - free_balances_before[&jurors_list[2]],
            21_600_219
        );

        let free_balance_after_3 = Balances::free_balance(jurors_list[3]);
        assert_eq!(
            free_balance_after_3 - free_balances_before[&jurors_list[3]],
            216_002_199
        );

        let free_balance_after_4 = Balances::free_balance(jurors_list[4]);
        assert_eq!(
            free_balance_after_4 - free_balances_before[&jurors_list[4]],
            2_160_021_996
        );
    });
}
...
fn check_pool_item_bob(params: Params) {
    let jurors = <CourtPool<Runtime>>::get();
    let pool_item_bob = jurors
        .iter()
        .find(|juror| juror.court_participant == BOB)
        .unwrap()
        .clone();
