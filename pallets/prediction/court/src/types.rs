...
/// Unique identifier for a court case.
/// 法庭案件的唯一标识。
pub type CourtId = u128;

/// The supported court vote categories.
/// 法庭支持的投票类别。
#[derive(
...
/// A court vote together with its concrete value.
/// 带有具体值的法庭投票。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// Helpers for converting concrete court votes.
/// 转换具体法庭投票的辅助方法。
impl VoteItem {
    /// Return the market outcome carried by this vote, if present.
    /// 如果该投票包含市场结果，则返回该结果。
...
/// General state for a court participant (juror or delegator).
/// 法庭参与者（陪审员或委托人）的通用状态。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// All possible states of a court vote.
/// 法庭投票的所有可能状态。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// The lifecycle status of a court case.
/// 法庭案件的生命周期状态。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// Information about an appeal in a court case.
/// 法庭案件中的申诉信息。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// Phase deadlines for one court round.
/// 一轮法庭审理各阶段的截止区块。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// Persistent information for a court case.
/// 法庭案件的持久化信息。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
    ...
    CourtInfo<BlockNumber, Appeals>
{
    ...
    pub(crate) fn update_round(&mut self, round_timing: RoundTiming<BlockNumber>) {
        ...
        self.round_ends.aggregation = self
            .round_ends
            .vote
            .saturating_add(round_timing.aggregation);
        self.round_ends.appeal = self
            .round_ends
            .aggregation
            .saturating_add(round_timing.appeal);
    }
}

/// A selected participant and the state needed after random drawing.
/// 随机抽取参与者后处理投票所需的状态。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
...
/// One entry in the stake-weighted juror pool.
/// 质押加权陪审员池中的一项。
#[derive(
    parity_scale_codec::Decode,
    parity_scale_codec::DecodeWithMemTracking,
