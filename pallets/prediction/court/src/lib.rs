...
use frame_support::{
    ...
    traits::{
        Currency, Get, Imbalance, LockIdentifier, LockableCurrency, NamedReservableCurrency,
...
mod pallet {
    ...
    /// Runtime dependencies and limits required by the court pallet.
    /// Court pallet 所需的运行时依赖与容量限制。
    #[pallet::config]
    pub trait Config: frame_system::Config {
        ...
        /// Generic runtime randomness source used to seed juror selection.
        /// 用于生成陪审员抽取种子的通用运行时随机源。
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            assert!(
                !T::BlocksPerYear::get().is_zero(),
                "Blocks per year musn't be zero!"
            );
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn join_court(
            ...
            Self::deposit_event(Event::JurorJoined {
                juror: who,
                stake: amount,
            });
        ...
        pub fn delegate(
            ...
            let mut sorted_delegations: DelegationsOf<T> = delegations
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::MaxDelegationsReached)?;
            ...
            let has_duplicates = sorted_delegations
                ...
                !sorted_delegations.contains(&who),
        ...
        pub fn prepare_exit_court(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            Self::deposit_event(Event::ExitPrepared {
        ...
        pub fn exit_court(
            ...
            let now = <frame_system::Pallet<T>>::block_number();
            let prepare_exit_at = prev_p_info
                .prepare_exit_at
                .ok_or(Error::<T>::PrepareExitAtNotPresent)?;
            ensure!(
            ...
            let (exit_amount, active_lock, weight) = if prev_p_info.active_lock.is_zero() {
                ...
                (
                    prev_p_info.stake,
                    BalanceOf::<T>::zero(),
                    T::WeightInfo::exit_court_remove(),
                )
            } else {
        ...
        pub fn vote(
            ...
            match draws.binary_search_by_key(&who, |draw| draw.court_participant.clone()) {
                Ok(index) => {
                    let draw = draws.get_mut(index).ok_or(Error::<T>::Unexpected(
        ...
        pub fn denounce_vote(
            ...
            if let Some(market_id) = <CourtIdToMarketId<T>>::get(court_id) {
                ...
                let outcome = vote_item
                    .clone()
                    .into_outcome()
                    .ok_or(Error::<T>::VoteItemIsNoOutcome)?;
                ensure!(
            ...
            let juror = T::Lookup::lookup(juror)?;
                ...
                <Participants<T>>::contains_key(&juror),
                Error::<T>::JurorDoesNotExist
            );
        ...
        pub fn appeal(
            ...
            let (new_round_ends, ids_len_1) = if appeal_number < T::MaxAppeals::get() as usize {
                ...
                debug_assert!(
                    request_block >= now,
                    "Request block must be greater than now."
                );
            ...
            Self::deposit_event(Event::CourtAppealed {
                court_id,
                appeal_info,
                new_round_ends,
            });

            Ok(Some(T::WeightInfo::appeal(
                pool_len,
                appeal_number,
                ids_len_0,
                ids_len_1,
            ...
            let mut handle_vote = |draw: DrawOf<T>| -> DispatchResult {
                match draw.vote {
                    ...
                    Vote::Revealed {
                        ...
                        jurors_to_stakes
                            .entry(draw.court_participant)
                            .or_default()
                            .self_info = Some(SelfInfo {
                            slashable: draw.slashable,
                            vote_item,
                        });
                    }
        ...
        T: Config,
    {
        fn get_uneligible_stake(
            ...
            if current_period_index != pool_item.uneligible_index {
                ...
                pool_item
                    .uneligible_stake
                    .checked_add_res(&additional_uneligible_stake)
            }
        }
        ...
        fn is_sorted_and_all_greater_than_lowest(
            ...
            p.windows(2)
                .all(|w| w[0].stake <= w[1].stake && lowest_stake <= w[0].stake)
        }
        ...
        fn do_join_court(
            ...
            delegations: Option<DelegationsOf<T>>,
                ...
                amount >= T::MinJurorStake::get(),
                Error::<T>::BelowMinJurorStake
            );
            ...
            let current_period_index =
                now.checked_div(&inflation_period)
                    .ok_or(Error::<T>::Unexpected(
                        UnexpectedError::InflationPeriodIsZero,
                    ))?;
        ...
        pub(crate) fn handle_inflation(now: BlockNumberFor<T>) -> Weight {
            ...
            let yearly_inflation_amount = yearly_inflation_rate
            ...
            let inflation_period_balance = inflation_period
                .saturated_into::<u128>()
            ...
            let current_period_index = now
                ...
                .map(|x| x.saturating_sub(One::one()));
        ...
        pub(crate) fn get_n_random_section_ends(n: usize, max: u128) -> BTreeSet<u128> {
            ...
            for i in 0..(n as u128) {
                ...
                let unused_random_number = *swaps
                    .get(&unused_random_index)
                    .unwrap_or(&unused_random_index);
        ...
        pub(crate) fn get_pool_item<'a>(
            ...
            if let Ok(i) = pool.binary_search_by_key(&(stake, court_participant), |pool_item| {
                ...
                let pool_item = pool.get(i).ok_or(Error::<T>::Unexpected(
                    UnexpectedError::BinarySearchByKeyFailed,
                ))?;
        ...
        fn slash_losers_to_award_winners(
            ...
            debug_assert!(
                missing.is_zero(),
                "Could not slash all of the amount for reward pot."
            );
            total_incentives.subsume(imb);
        ...
        pub(crate) fn get_latest_winner_vote_item(
            ...
            let report = market
                .report
                .as_ref()
                .ok_or(Error::<T>::MarketReportNotFound)?;
        ...
        pub(crate) fn get_hashed_commitment(
            ...
            match vote {
                ...
                Vote::Delegated {
                    delegated_stakes: _,
                } => Err(Error::<T>::JurorDelegated.into()),
                Vote::Revealed {
                    commitment: _,
                    vote_item: _,
                    salt: _,
        ...
        T: Config,
    {
        ...
        fn on_dispute(
            ...
            let next_court_id = court_id
                .checked_add(One::one())
                .ok_or(Error::<T>::MaxCourtIdReached)?;
            ...
            Self::deposit_event(Event::CourtOpened {
                market_id: *market_id,
                court_info: court,
            });
        ...
        fn on_resolution(
            ...
            court.status = CourtStatus::Closed {
                winner: winner_vote_item.clone(),
            };
            <Courts<T>>::insert(court_id, court);

            let winner_outcome = winner_vote_item
                .into_outcome()
                .ok_or(Error::<T>::WinnerVoteItemIsNoOutcome)?;
        ...
        fn exchange(
            ...
            // Global-dispute escalation removes the draws but retains the court until this
            // outcome-dependent appeal-bond exchange has completed.
            // 全局争议升级会删除抽签记录，但保留 Court，直到依赖最终结果的上诉保证金
            // 结算完成。
            if !<SelectedDraws<T>>::contains_key(court_id) {
                <Courts<T>>::remove(court_id);
                <MarketIdToCourtId<T>>::remove(market_id);
                <CourtIdToMarketId<T>>::remove(court_id);
            }
        ...
        fn has_failed(
            ...
            let pool_unconsumed_stake = pool.iter().fold(0u128, |acc, pool_item| {
                ...
                let remainder = unconsumed
                    .checked_rem_res(&min_juror_stake)
            ...
            let res = ResultWithWeightInfo {
                result: has_failed,
                weight: T::WeightInfo::has_failed(),
            };
        ...
        fn clear(
            ...
            let res = ResultWithWeightInfo {
                result: (),
                weight: T::WeightInfo::clear(draws_len),
            };
