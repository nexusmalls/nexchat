...
fn it_appeals_a_court_market_to_global_dispute() {
    let test = |base_asset: AssetOf<Runtime>| {
        ...
        let resolved_market = MarketCommons::market(&market_id).unwrap();
        assert_eq!(resolved_market.status, MarketStatus::Resolved);
        assert_eq!(
            resolved_market.resolved_outcome,
            Some(OutcomeReport::Categorical(0))
        );
        assert!(!GlobalDisputes::is_active(&market_id));
        assert!(MarketIdsPerDisputeBlock::<Runtime>::get(vote_end).is_empty());
        assert!(zrml_court::MarketIdToCourtId::<Runtime>::get(market_id).is_none());
        assert_eq!(Balances::reserved_balance(BOB), 0);
        assert_eq!(Balances::free_balance(BOB), bob_free_before_appeals);
    };
    ExtBuilder::default().build().execute_with(|| {
        test(Asset::Ztg);
    });
    ExtBuilder::default().build().execute_with(|| {
        test(Asset::ForeignAsset(USDX_ASSET_ID));
    });
}
...
fn the_entire_market_lifecycle_works_with_timestamps() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let grace_period = u64::from(market.deadlines.grace_period) * MILLISECS_PER_BLOCK as u64;
...
fn authorized_correctly_resolves_disputed_market() {
    ...
    let test = |base_asset: AssetOf<Runtime>| {
        ...
        if base_asset == Asset::Ztg {
            ...
            assert_eq!(
                charlie_balance,
                1_000 * BASE - <Runtime as Config>::DisputeBond::get()
            );
            let charlie_balance = AssetManager::free_balance(base_asset, &CHARLIE);
            assert_eq!(charlie_balance, 1_000 * BASE - CENT);
        }
        ...
        if base_asset == Asset::Ztg {
            ...
            assert_eq!(
                charlie_balance,
                1_000 * BASE + <Runtime as Config>::OracleBond::get()
            );
            let charlie_balance = AssetManager::free_balance(base_asset, &CHARLIE);
            assert_eq!(charlie_balance, 1_000 * BASE - CENT);
        }
...
fn it_resolves_a_disputed_court_market() {
    let test = |base_asset: AssetOf<Runtime>| {
        ...
        assert_eq!(
            free_juror_0_after,
            free_juror_0_before + juror_0_share * total_slashed
        );
        let juror_2_share = Perquintill::from_rational(slashable_juror_2, total_winner_stake);
        assert_eq!(
            free_juror_2_after,
            free_juror_2_before + juror_2_share * total_slashed
        );
    };
    ExtBuilder::default().build().execute_with(|| {
        test(Asset::Ztg);
