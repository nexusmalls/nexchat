...
fn sudo_schedule_early_close_at_timeframe_works() {
    ExtBuilder::default().build().execute_with(|| {
        let start_block: BlockNumber = 7;
        set_timestamp_for_on_initialize(u64::from(start_block) * MILLISECS_PER_BLOCK as u64);
        ...
        set_timestamp_for_on_initialize(
            u64::from(start_block) * MILLISECS_PER_BLOCK as u64 + new_end,
        );
        run_to_block(
            start_block
                + BlockNumber::try_from(new_end.saturating_div(MILLISECS_PER_BLOCK.into()))
                    .expect("mock timestamp fits BlockNumber")
                + 1,
        );
...
fn schedule_early_close_timestamp_fails_if_early_close_request_too_late() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        run_to_block(
            BlockNumber::try_from(end.saturating_div(MILLISECS_PER_BLOCK.into()))
                .expect("mock timestamp fits BlockNumber")
                - 1,
        );
