...
#[test]
fn create_market_enforces_live_collateral_policy() {
    ExtBuilder::default().build().execute_with(|| {
        let create = |base_asset| {
            PredictionMarkets::create_market(
                RuntimeOrigin::signed(ALICE),
                base_asset,
                Perbill::zero(),
        ...
        assert_ok!(create(Asset::Ztg));
        assert_ok!(create(Asset::ForeignAsset(USDX_ASSET_ID)));
        assert_noop!(
            create(Asset::ForeignAsset(50)),
            Error::<Runtime>::InvalidBaseAsset,
        );

        assert_ok!(PredictionControl::set_prediction_mode(
            RuntimeOrigin::root(),
            pallet_prediction_control::PredictionMode::Trading,
        ));
        assert_noop!(
            create(Asset::ForeignAsset(USDX_ASSET_ID)),
            Error::<Runtime>::InvalidBaseAsset,
        );
        assert_ok!(PredictionControl::set_prediction_mode(
            RuntimeOrigin::root(),
        ...
        assert_eq!(
            pallet_assets::Asset::<Runtime>::get(USDX_ASSET_ID)
                .expect("USDX exists")
                .status,
            pallet_assets::AssetStatus::Frozen,
        );
        assert_noop!(
            create(Asset::ForeignAsset(USDX_ASSET_ID)),
            Error::<Runtime>::InvalidBaseAsset,
...
fn create_market_suceeds_if_market_duration_is_maximal_in_moments() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let end = now
            + u64::from(<Runtime as Config>::MaxMarketLifetime::get())
...
fn create_market_fails_if_market_duration_is_too_long_in_moments() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let start = 5 * MILLISECS_PER_BLOCK as u64;
        let end = now
            + u64::from(<Runtime as Config>::MaxMarketLifetime::get() + 1)
                * (MILLISECS_PER_BLOCK as u64);
        assert!(
            end > start,
            "Test failed due to misconfiguration: `MaxMarketLifetime` is too small"
