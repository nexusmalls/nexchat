...
//! Prediction market lifecycle, complete-set, reporting, dispute, and settlement logic.
//! 预测市场生命周期、完整份额集、报告、争议与结算逻辑。
...
mod pallet {
    ...
    use frame_support::{
        ...
        traits::{
            tokens::BalanceStatus, Currency, EnsureOrigin, ExistenceRequirement, Get, Hooks,
            Imbalance, NamedReservableCurrency, OnUnbalanced, StorageVersion,
        },
        transactional,
        weights::Weight,
    ...
    use zeitgeist_primitives::{
        ...
        traits::{
            ...
            DisputeResolutionApi, MarketBuilderTrait, PayoutApi, PredictionBaseAssetPolicy,
        },
        types::{
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn start_global_dispute(
            ...
            // Clear the mechanism-owned schedule before `on_global_dispute` is allowed to
            // remove its state; otherwise the stale entry can later fail with `CourtNotFound`.
            // 在 `on_global_dispute` 删除争议机制状态前清理其自动决议队列，否则残留项
            // 会在旧决议块触发 `CourtNotFound`。
    ...
    pub trait Config: frame_system::Config + zrml_market_commons::Config {
        ...
        /// Policy for admitting foreign assets as market collateral.
        /// 外部资产作为市场抵押品的准入策略。
        type BaseAssetPolicy: PredictionBaseAssetPolicy<u64>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn construct_market(
            ...
            let valid_base_asset = match base_asset {
                Asset::Ztg => true,
                Asset::ForeignAsset(asset_id) => T::BaseAssetPolicy::is_allowed(asset_id),
                _ => false,
            };
        ...
        fn payout_vector(market_id: Self::MarketId) -> Option<Vec<Self::Balance>> {
            ...
            let result = match resolved_outcome {
                ...
                OutcomeReport::Scalar(value) => {
                    ...
                    let payout_short = ZeitgeistBase::<BalanceOf<T>>::get()
                        .ok()?
