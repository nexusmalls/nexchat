...
mod pallet {
    ...
    use frame_support::{ensure, require_transactional, traits::StorageVersion, PalletId};
    use frame_system::{
        ensure_signed,
    ...
    #[pallet::config]
    /// Runtime boundaries used to route trades across Neo Swaps and the order book.
    ///
    /// 用于在 Neo Swaps 与订单簿之间路由交易的 runtime 边界。
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        /// The API used to custody and transfer all supported asset classes.
        ///
        /// 用于托管和转移所有受支持资产类别的 API。
        type AssetManager: MultiCurrency<Self::AccountId, CurrencyId = AssetOf<Self>>;

        /// Pool deployment API used only by generated benchmarks.
        ///
        /// 仅供生成式基准测试使用的资金池部署 API。
        #[cfg(feature = "runtime-benchmarks")]
        ...
        /// Complete-set operation API used only by generated benchmarks.
        ///
        /// 仅供生成式基准测试使用的完整集合操作 API。
        #[cfg(feature = "runtime-benchmarks")]
        ...
        /// The market storage boundary and market identifier source.
        ///
        /// 市场存储边界及市场标识来源。
        ...
        /// The API boundary for the Automated Market Maker (AMM).
        ///
        /// 自动做市商（AMM）的 API 边界。
        ...
        /// The maximum number of order-book orders considered by one routed trade.
        ///
        /// 单次路由交易可考虑的订单簿订单数量上限。
        #[pallet::constant]
        type MaxOrders: Get<u32>;

        /// The API boundary for order lookup, fill, and placement.
        ///
        /// 用于查询、成交及挂单的订单簿 API 边界。
        ...
        /// Pallet account identifier used by cross-pallet operations.
        ///
        /// 跨 pallet 操作使用的 pallet 账户标识。
    ...
    impl<T: Config> Pallet<T> {
        ...
        ///
        /// 将买单路由至 AMM 与连续双向拍卖订单簿，以获得最佳平均执行价格。
        ///
        /// # Parameters
        ///
        ...
        ///
        /// 将卖单路由至 AMM 与连续双向拍卖订单簿，以获得最佳平均执行价格。
        ///
        /// # Parameters
        ///
        ...
        T: Config,
    {
                ...
                match tx_type {
                    TxType::Buy => {
                        // existing order is willing to give the required `asset` as the `maker_asset`
                            ...
                            order_price <= price_limit,
                            Error::<T>::OrderPriceAboveMaxPrice
                        );
                    }
                    TxType::Sell => {
                        // existing order is willing to receive the required `asset` as the `taker_asset`
                            ...
                            order_price >= price_limit,
                            Error::<T>::OrderPriceBelowMinPrice
                        );
                    }
                }
        ...
        pub(crate) fn do_trade(
            ...
            let assets = market.outcome_assets();
                ...
                asset_count as usize == assets.len(),
                Error::<T>::AssetCountMismatch
            );
        ...
        fn get_event_info(
            ...
            orderbook_trades
                ...
                .try_fold(
                    TradeEventInfo::<T>::new(),
                    |event_info: TradeEventInfo<T>, trade| {
