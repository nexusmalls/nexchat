...
pub enum Strategy {
    ...
    ///
    /// 尽可能部分成交，并将剩余数量挂入订单簿；优先满足目标价格而非立即成交。
    LimitOrder,
}

/// Identifies whether a routed trade buys or sells the outcome asset.
///
/// 标识路由交易是买入还是卖出结果资产。
#[derive(Clone, Copy, Debug, Decode, DecodeWithMemTracking, Encode, PartialEq, TypeInfo)]
pub enum TxType {
    Buy,
    Sell,
}
...
impl<T: Config> TradeEventInfo<T> {
    ...
    pub fn add_amount_out_minus_fees(&mut self, additional: Self) -> Result<(), DispatchError> {
        self.external_fee_amount = self
            .external_fee_amount
            .checked_add_res(&additional.external_fee_amount)?;
        self.swap_fee_amount = self
            .swap_fee_amount
            .checked_add_res(&additional.swap_fee_amount)?;
        let fees = additional
            .external_fee_amount
            .checked_add_res(&additional.swap_fee_amount)?;
