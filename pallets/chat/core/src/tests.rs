...
fn test_send_message_rejects_cid_too_long() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Chat::send_message(RuntimeOrigin::signed(ALICE), BOB, too_long_cid, 4, None),
...
fn test_notify_bypasses_permission_gate() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Chat as SystemNotifier<u64>>::notify(
            &BOB,
            b"sys:1".to_vec()
        ));
        assert_eq!(Chat::get_message(0).unwrap().msg_type, MessageType::System);
...
fn test_cleanup_old_messages_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(Event::OldMessagesCleanedUp { count: 2 }.into());
    });
}
...
fn session_mute_set_and_clear() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Chat::set_session_muted(
            RuntimeOrigin::signed(ALICE),
            sid,
            true
        ));
...
fn session_pin_set_and_clear() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Chat::set_session_pinned(
            RuntimeOrigin::signed(ALICE),
            sid,
            true
        ));
