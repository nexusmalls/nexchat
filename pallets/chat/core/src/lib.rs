...
use frame_support::{
    pallet_prelude::*,
    traits::{EnsureOrigin, Randomness, UnixTime},
    BoundedVec,
};
...
pub mod pallet {
    ...
    pub type Messages<T: Config> = StorageMap<_, Blake2_128Concat, u64, MessageMeta<T>>;
    ...
    pub type UnreadCount<T: Config> =
        StorageMap<_, Blake2_128Concat, (T::AccountId, T::Hash), u32, ValueQuery>;
    ...
    /// - 用于防止ID重复
    #[pallet::storage]
    pub type UsedChatUserIds<T: Config> =
        StorageMap<_, Blake2_128Concat, ChatUserId, bool, OptionQuery>;
    ...
    pub type AccountToChatUserId<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, ChatUserId, OptionQuery>;
    ...
    pub type ChatUserIdToAccount<T: Config> =
        StorageMap<_, Blake2_128Concat, ChatUserId, T::AccountId, OptionQuery>;
    ...
    /// - 包含昵称、头像、状态等信息
    #[pallet::storage]
    pub type ChatUserProfiles<T: Config> =
        StorageMap<_, Blake2_128Concat, ChatUserId, ChatUserProfile<T>, OptionQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// 函数级详细中文注释：消息已读
        /// [msg_id, reader]
        MessageRead { msg_id: u64, reader: T::AccountId },

        /// 函数级详细中文注释：消息已删除
        /// [msg_id, deleter]
        MessageDeleted { msg_id: u64, deleter: T::AccountId },

        /// 消息已被发送方撤回（双方隐藏）。
        /// Message recalled by its sender (hidden for both parties).
        /// [msg_id, session_id]
        MessageRecalled { msg_id: u64, session_id: T::Hash },
        ...
        /// 函数级详细中文注释：聊天用户资料更新
        /// [chat_user_id]
        ChatUserProfileUpdated { chat_user_id: ChatUserId },
        ...
        /// 函数级详细中文注释：隐私设置更新
        /// [chat_user_id]
        PrivacySettingsUpdated { chat_user_id: ChatUserId },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn mark_as_read(origin: OriginFor<T>, msg_id: u64) -> DispatchResult {
        ...
        pub fn recall_message(origin: OriginFor<T>, msg_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            let (session_id, was_unread, receiver) = Messages::<T>::try_mutate(
                msg_id,
                |maybe_msg| -> Result<(T::Hash, bool, T::AccountId), DispatchError> {
                    let msg = maybe_msg.as_mut().ok_or(Error::<T>::MessageNotFound)?;
                    ...
                    let elapsed = now.saturating_sub(msg.sent_at);
                        ...
                        elapsed <= T::MessageRecallWindow::get(),
                        Error::<T>::RecallWindowExpired
                    );
        ...
        pub fn set_session_muted(
            ...
            Self::deposit_event(Event::SessionMuteSet {
                session_id,
                user: who,
                muted,
            });
        ...
        pub fn set_session_pinned(
            ...
            Self::deposit_event(Event::SessionPinSet {
                session_id,
                user: who,
                pinned,
            });
        ...
        pub fn cleanup_old_messages(origin: OriginFor<T>, limit: u32) -> DispatchResult {
            ...
            while scanned < limit {
                match iter.next() {
                    Some((msg_id, msg)) => {
                        ...
                        let both_deleted = msg.is_deleted_by_sender && msg.is_deleted_by_receiver;
                        let system_receiver_deleted =
                            msg.sender == system_account && msg.is_deleted_by_receiver;
        ...
        pub fn update_chat_profile(
            ...
            let avatar_cid_bounded = if let Some(cid_vec) = avatar_cid {
                ...
                Some(Some(
                    cid_vec.try_into().map_err(|_| Error::<T>::CidTooLong)?,
                ))
            } else {
                None
            };
            ...
            // 触发事件
            Self::deposit_event(Event::ChatUserProfileUpdated { chat_user_id });
        ...
        pub fn set_user_status(origin: OriginFor<T>, status_code: u8) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn update_privacy_settings(
            ...
            // 触发事件
            Self::deposit_event(Event::PrivacySettingsUpdated { chat_user_id });
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_send(
            ...
            if !is_system {
                ...
                // Single permission gate: chat-permission is the sole source of truth.
                    ...
                    <T::ChatPermission as pallet_chat_permission::ChatPermissionChecker<
                        T::AccountId,
                    >>::can_send_message(&sender, &receiver),
            ...
            // is guaranteed solely by client-side MLS E2EE; the chain stores an opaque CID.
                ...
                content_cid.len() <= T::MaxCidLen::get() as usize,
                Error::<T>::CidTooLong
            );

            let cid_bounded: BoundedVec<u8, T::MaxCidLen> =
        ...
        pub fn create_session(
            ...
            // 创建新会话
            let now = <frame_system::Pallet<T>>::block_number();
            let participants_bounded: BoundedVec<T::AccountId, ConstU32<2>> = participants
                .clone()
                .try_into()
        ...
        fn ensure_session_participant(who: &T::AccountId, session_id: T::Hash) -> DispatchResult {
            let session = Sessions::<T>::get(session_id).ok_or(Error::<T>::SessionNotFound)?;
            ensure!(
                session.participants.contains(who),
                Error::<T>::NotSessionParticipant
        ...
        /// - None: 不存在对应关系
        pub fn get_account_by_chat_user_id(chat_user_id: ChatUserId) -> Option<T::AccountId> {
        ...
        /// - None: 尚未注册聊天用户
        pub fn get_chat_user_id_by_account(account: &T::AccountId) -> Option<ChatUserId> {
        ...
        /// - None: 用户不存在
        pub fn get_chat_user_profile(chat_user_id: ChatUserId) -> Option<ChatUserProfile<T>> {
        ...
        pub fn get_session_id(account1: &T::AccountId, account2: &T::AccountId) -> T::Hash {
            // 确保账户顺序一致，生成确定性的会话ID
