...
- **E2E（真实节点，待联调）**：现有 `scripts/e2e/` runner（`scripts/docs/NEXUS_TEST_PLAN.md`）仅覆盖**链上 pallet** smoke（entity/commission/prediction…），**无** MLS/relay 客户端 harness；群 Wire 客户端行为在本仓一律走 WASM vitest（见上）。剩余真实节点联调（起 `nexus-node` 多端验证并发群发 / 换机被 Add / 移除自愈在真链上的端到端时序）属节点依赖步骤，本轮未在无节点环境执行。relay 路由 + appStore 实时会话前置均已就绪（见上）。
