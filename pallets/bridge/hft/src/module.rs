...
//! Hyper Fungible Token pallet 的 ISMP module 实现。
        ...
        From<<<T as Config>::NativeCurrency as Currency<T::AccountId>>::Balance>,
{
    fn on_accept(
        ...
        .map_err(|error| HftError::InvalidAmountConversion(format!("{error:?}")))?;
        ...
        if local_asset_id == T::NativeAssetId::get() {
            ...
            .map_err(HftError::TransferFailed)?;
        } else if NativeAssets::<T>::get(local_asset_id.clone()) {
        ...
        if !message.data.is_empty() {
            ...
            let origin = if let Some(signature) = substrate_data.signature {
                ...
                match multi_signature {
                    MultiSignature::Ed25519(signature) => {
                        ...
                        let public_key = beneficiary_bytes
                            .as_slice()
                            .try_into()
                            .map_err(|_| HftError::SignatureVerificationFailed)?;
                        if !sp_io::crypto::ed25519_verify(&signature, message.as_ref(), &public_key)
                        {
                    ...
                    MultiSignature::Sr25519(signature) => {
                        ...
                        let public_key = beneficiary_bytes
                            ...
                            .try_into()
                            .map_err(|_| HftError::SignatureVerificationFailed)?;
                        if !sp_io::crypto::sr25519_verify(&signature, message.as_ref(), &public_key)
                        {
                            Err(HftError::SignatureVerificationFailed)?
                        }
                    }
                    MultiSignature::Ecdsa(signature) => {
                        ...
                        let public_key =
                            sp_io::crypto::secp256k1_ecdsa_recover(&signature.0, &message)
                                .map_err(|_| HftError::EcdsaRecoveryFailed)?;
                        let evm_address =
                            H160::from_slice(&sp_io::hashing::keccak_256(&public_key)[12..]);
                        if T::EvmToSubstrate::convert(evm_address) != beneficiary {
                            Err(HftError::SignatureVerificationFailed)?
                        }
                    }
            ...
            runtime_call
                ...
                .map_err(|error| HftError::CallDispatchError(error.error))?;
