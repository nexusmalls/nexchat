...
use zeitgeist_primitives::types::{Asset as PredictionAsset, OutcomeReport, SerdeWrapper};

// Local module imports
use super::{
    AccountId, Arbitration, Babe, Balance, Block, BlockNumber, ChatCore, ChatGroup, ChatInbox,
    ChatPermission, ChatSync, CommissionPoolReward, EntityMarket, EntityRegistry, Evidence,
    Executive, Grandpa, Hash, Historical, InherentDataExt, MsgIdentity, NexMarket, Nonce,
    PredictionCollateral, PredictionControl, PredictionCurrencies, PredictionLegacySwaps,
    PredictionNeoSwaps, Runtime, RuntimeCall, RuntimeGenesisConfig, SessionKeys, StorageService,
    System, TransactionPayment, VERSION,
};

fn prediction_asset_view(asset: PredictionAsset<u128>) -> PredictionAsset<SerdeWrapper<u128>> {
    match asset {
        PredictionAsset::CategoricalOutcome(market_id, category) => {
            PredictionAsset::CategoricalOutcome(SerdeWrapper(market_id), category)
        }
        PredictionAsset::ScalarOutcome(market_id, position) => {
            PredictionAsset::ScalarOutcome(SerdeWrapper(market_id), position)
        }
        PredictionAsset::CombinatorialOutcomeLegacy => PredictionAsset::CombinatorialOutcomeLegacy,
        PredictionAsset::PoolShare(pool_id) => PredictionAsset::PoolShare(pool_id),
        PredictionAsset::Ztg => PredictionAsset::Ztg,
        PredictionAsset::ForeignAsset(asset_id) => PredictionAsset::ForeignAsset(asset_id),
        PredictionAsset::ParimutuelShare(market_id, category) => {
            PredictionAsset::ParimutuelShare(SerdeWrapper(market_id), category)
        }
        PredictionAsset::CombinatorialToken(id) => PredictionAsset::CombinatorialToken(id),
    }
}

fn prediction_outcome_view(
    outcome: OutcomeReport,
) -> zrml_prediction_markets_runtime_api::OutcomeView {
    match outcome {
        OutcomeReport::Categorical(category) => {
            zrml_prediction_markets_runtime_api::OutcomeView::Categorical(category)
        }
        OutcomeReport::Scalar(value) => {
            zrml_prediction_markets_runtime_api::OutcomeView::Scalar(SerdeWrapper(value))
        }
    }
}
...
impl_runtime_apis! {
    ...
    impl zrml_swaps_runtime_api::SwapsApi<Block, u128, AccountId, Balance, u128> for Runtime {
        fn pool_shares_id(
            pool_id: u128,
        ) -> zeitgeist_primitives::types::Asset<
            zeitgeist_primitives::types::SerdeWrapper<u128>,
        > {
            zeitgeist_primitives::types::Asset::PoolShare(pool_id)
        }

        fn pool_account_id(pool_id: &u128) -> AccountId {
            PredictionLegacySwaps::pool_account_id(pool_id)
        }

        fn get_spot_price(
            pool_id: &u128,
            asset_in: &zeitgeist_primitives::types::Asset<u128>,
            asset_out: &zeitgeist_primitives::types::Asset<u128>,
            with_fees: bool,
        ) -> zeitgeist_primitives::types::SerdeWrapper<Balance> {
            zeitgeist_primitives::types::SerdeWrapper(
                PredictionLegacySwaps::get_spot_price(
                    pool_id,
                    asset_in,
                    asset_out,
                    with_fees,
    ...
    impl zrml_prediction_markets_runtime_api::PredictionMarketsApi<Block, u128, Hash> for Runtime {
        fn market_outcome_share_id(
            market_id: u128,
            outcome: u16,
        ) -> zeitgeist_primitives::types::Asset<u128> {
            zeitgeist_primitives::types::Asset::CategoricalOutcome(market_id, outcome)
        }
    }

    impl zrml_prediction_markets_runtime_api::PredictionViewApi<Block, AccountId> for Runtime {
        fn market_summary(
            market_id: u128,
        ) -> Option<zrml_prediction_markets_runtime_api::MarketSummary<AccountId>> {
            use zeitgeist_primitives::{
                traits::HybridRouterAmmApi,
                types::{
                    MarketDisputeMechanism, MarketPeriod, MarketStatus, MarketType, ScoringRule,
                },
            };
            use zrml_prediction_markets_runtime_api::{
                MarketPeriodView, MarketSummary, ReportView,
            };

            let market = zrml_market_commons::Markets::<Runtime>::get(market_id)?;
            let outcome_assets = market
                .outcome_assets()
                .into_iter()
                .map(prediction_asset_view)
                .collect();
            let status = match market.status {
                MarketStatus::Proposed => 0,
                MarketStatus::Active => 1,
                MarketStatus::Closed => 2,
                MarketStatus::Reported => 3,
                MarketStatus::Disputed => 4,
                MarketStatus::Resolved => 5,
            };
            let scoring_rule = match market.scoring_rule {
                ScoringRule::AmmCdaHybrid => 0,
                ScoringRule::Parimutuel => 1,
            };
            let (market_type, scalar_low, scalar_high) = match &market.market_type {
                MarketType::Categorical(_) => (0, None, None),
                MarketType::Scalar(range) => (
                    1,
                    Some(SerdeWrapper(*range.start())),
                    Some(SerdeWrapper(*range.end())),
                ),
            };
            let period = match market.period {
                MarketPeriod::Block(range) => MarketPeriodView::Block {
                    start: range.start,
                    end: range.end,
                },
                MarketPeriod::Timestamp(range) => MarketPeriodView::Timestamp {
                    start: range.start,
                    end: range.end,
                },
            };
            let report = market.report.map(|report| ReportView {
                at: report.at,
                by: report.by,
                outcome: prediction_outcome_view(report.outcome),
            });
            let dispute_mechanism = market.dispute_mechanism.map(|mechanism| match mechanism {
                MarketDisputeMechanism::Authorized => 0,
                MarketDisputeMechanism::Court => 1,
            });
            let canonical_pool =
                <PredictionNeoSwaps as HybridRouterAmmApi>::pool_exists(market_id)
                    .then_some(SerdeWrapper(market_id));

            Some(MarketSummary {
                market_id: SerdeWrapper(market.market_id),
                base_asset: prediction_asset_view(market.base_asset),
                ...
                scalar_high,
                period,
                grace_period: market.deadlines.grace_period,
                oracle_duration: market.deadlines.oracle_duration,
                dispute_duration: market.deadlines.dispute_duration,
                outcome_assets,
                report,
                resolved_outcome: market.resolved_outcome.map(prediction_outcome_view),
                dispute_mechanism,
                canonical_pool,
            })
        }
        ...
        fn spot_prices(
            market_id: u128,
        ) -> Vec<zrml_prediction_markets_runtime_api::SpotPriceView> {
            use zeitgeist_primitives::traits::HybridRouterAmmApi;
            use zrml_prediction_markets_runtime_api::SpotPriceView;

            let Some(market) = zrml_market_commons::Markets::<Runtime>::get(market_id) else {
                return Vec::new();
            };
            market
                .outcome_assets()
                .into_iter()
                .filter_map(|asset| {
                    <PredictionNeoSwaps as HybridRouterAmmApi>::get_spot_price(market_id, asset)
                        .ok()
                        .map(|price| SpotPriceView {
                            asset: prediction_asset_view(asset),
                            price: SerdeWrapper(price),
                        })
                })
                .collect()
        }
        ...
        fn user_redeemable(market_id: u128, account: AccountId) -> SerdeWrapper<u128> {
            ...
            let Some(market) = zrml_market_commons::Markets::<Runtime>::get(market_id) else {
                return SerdeWrapper(0);
            };
            if market.status != MarketStatus::Resolved || !market.is_redeemable() {
                return SerdeWrapper(0);
            }
            let Some(payouts) = <crate::PredictionMarkets as PayoutApi>::payout_vector(market_id)
            else {
                return SerdeWrapper(0);
            };
            let redeemable = market
                .outcome_assets()
                .into_iter()
                .zip(payouts)
                .fold(0u128, |total, (asset, payout)| {
                    let balance =
                        <PredictionCurrencies as MultiCurrency<AccountId>>::free_balance(
                            asset,
                            &account,
                        );
                    let value = (sp_core::U256::from(balance) * sp_core::U256::from(payout)
                        / sp_core::U256::from(zeitgeist_primitives::constants::BASE))
                    .as_u128();
                    total.saturating_add(value)
                });
            SerdeWrapper(redeemable)
        }

        fn court_summary(
            market_id: u128,
        ) -> Option<zrml_prediction_markets_runtime_api::CourtCaseSummary> {
            use zrml_court::types::CourtStatus;
            use zrml_prediction_markets_runtime_api::CourtCaseSummary;

            let court_id = zrml_court::MarketIdToCourtId::<Runtime>::get(market_id)?;
            let court = zrml_court::Courts::<Runtime>::get(court_id)?;
            let status = match court.status {
                CourtStatus::Open => 0,
                CourtStatus::Closed { .. } => 1,
                CourtStatus::Reassigned => 2,
            };
            Some(CourtCaseSummary {
                court_id: SerdeWrapper(court_id),
                status,
                appeals: court.appeals.len() as u32,
                pre_vote_end: court.round_ends.pre_vote,
                vote_end: court.round_ends.vote,
                aggregation_end: court.round_ends.aggregation,
                appeal_end: court.round_ends.appeal,
            })
        }

        fn collateral_mirror_status(
            asset_id: u64,
        ) -> zrml_prediction_markets_runtime_api::CollateralMirrorStatus {
            use zrml_prediction_markets_runtime_api::CollateralMirrorStatus;

            CollateralMirrorStatus {
                asset_id,
                whitelisted:
                    pallet_prediction_collateral::WhitelistedAssets::<Runtime>::get(asset_id)
                        == Some(true),
                global_deposit_paused: PredictionCollateral::global_deposit_paused(),
                asset_deposit_paused: PredictionCollateral::asset_deposit_paused(asset_id),
                deposit_allowed: PredictionCollateral::is_deposit_allowed(asset_id),
                mirror_issuance: SerdeWrapper(PredictionCollateral::mirror_issuance(asset_id)),
                escrow_balance: SerdeWrapper(PredictionCollateral::escrow_balance(asset_id)),
                consistent: PredictionCollateral::is_mirror_consistent(asset_id),
            }
        }

        fn control_status() -> zrml_prediction_markets_runtime_api::PredictionControlStatus {
            use pallet_prediction_control::{PredictionMode, PredictionModule};
            use zrml_prediction_markets_runtime_api::PredictionControlStatus;

            let mode = match PredictionControl::prediction_mode() {
                PredictionMode::Disabled => 0,
                PredictionMode::ResolutionOnly => 1,
                PredictionMode::Trading => 2,
                PredictionMode::Full => 3,
            };
            let modules = [
            ...
            let enabled_modules =
                modules
                    .into_iter()
                    .enumerate()
                    .fold(0u16, |bits, (index, module)| {
                        if PredictionControl::module_enabled(module) {
                            bits | (1u16 << index)
                        } else {
                            bits
                        }
                    });
            PredictionControlStatus {
                mode,
                enabled_modules,
            }
        }
